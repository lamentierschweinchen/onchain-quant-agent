...
**Last updated**: 2026-09-21 (run #26, schema v2)
...
**3. Score the record, and keep the two numbers apart.** Each run appends its prediction record to `running_baselines`: `pre_committed_tests_resolved`, `pre_committed_test_hit_rate_pct`, `pre_committed_tests_open`. The hit rate is `as_predicted / resolved`. Report it two ways when they differ, because they measure different things - the rate over tests that actually RESOLVED is prediction quality; the rate including `withdrawn` premises also charges the model for publishing claims that did not survive deeper measurement. Run #21 baseline: 3 of 5 resolved tests fired as predicted, or 3 of 7 counting the two withdrawn premises (42.9%).

The dashboard aggregates the same figures at build time into `public/scoreboard.json` (per-run record plus a cross-run ledger, so an open test stays visible until it settles). A rate over fewer than ~12 resolved tests is a record being built, not a rate to trust, and the panel says so.
...
### THE PROVIDER JOIN KEY IS IDENTITY, NOT ADDRESS - AND THE MISMATCH FAILS SILENTLY (run #22, 2026-08-24)

`data/previous.json` stores `staking_providers[].provider` as the provider's **identity** (falling back to the contract address only when a provider has no identity). The live `/providers` response keys on `provider` = the **contract address**. Joining the two on `provider` therefore matches almost nothing.

Run #22's collector did exactly that. It reported **79 of 105 providers as having moved more than 20,000 EGLD**, found **zero** unDelegate callers, and wrote an empty unbonding pool. Nothing errored. The only reason it was caught is that 79 large movers is implausible on its face; the true count at a 5,000 EGLD threshold is **ten**.

**Rules**:
1. Join on `identity or provider`, both sides.
2. Assert a minimum match rate on every cross-snapshot join (>80% of the smaller set). A join that silently degrades to near-zero matches is indistinguishable from a quiet week.

### A DEREGISTERED PROVIDER DOES NOT LEAVE /providers (run #22, 2026-08-24)

When an operator calls `unStakeNodes`, the provider stays in the API with `numNodes` unchanged, `numUsers` unchanged, and `stake`/`topUp`/`locked` all zero with `apr: 0`. Run #22: p2p_org_ went from 67,500 stake + 2,083 topUp to zero while still listing **50 nodes and 1,244 delegators**.

Any "providers leaving" count built on set membership misses this entirely. **Detection signature: `locked == 0` with `numNodes > 0`**, confirmed by an `unStakeNodes` call on the contract's inbound transaction list (it appears there as a normal function alongside delegate/unDelegate/claimRewards).

**Consequences that must be netted out before narrating anything**:
- Its full prior `locked` drops out of delegation TVL in one step (87% of run #22's -80,367).
- Its `numUsers` drop out of the delegator count **without anyone undelegating**, because the working set is providers with `locked > 0`. Run #22: 1,244 of an apparent -1,311 "break" in a nine-week flat series. Report the count both ways.

### AN UNBONDING CAN COMPLETE AND STILL NOT MOVE - DESTINATION TESTS NEED A NO-ACTION BRANCH (run #22, 2026-08-24)

`/accounts/{addr}/delegation` reports a finished unbonding as `userUnBondable > 0` with a `userUndelegatedList` entry at `seconds: 0`. The EGLD is claimable but has **not left the contract**: it is absent from the wallet balance, absent from the provider's `locked`, and present inside `economics.staked`.

Run #21 registered a three-branch destination test on 229,865 EGLD (delegation contract / exchange / wallet). Run #22 found a **fourth state none of them covered** - the holder simply did not withdraw. The test could not resolve and was recorded `inconclusive`.

**Rule**: every destination test needs an explicit **no-action branch with a time bound** ("still unmoved after N weeks = an inactive holder, retire it as a live flow"). Without one, a counterparty doing nothing produces an unresolvable test rather than an answer.

### NARROWING RUN #21's WEEKLY-NETTING RULE - TEST THE WAVE, DO NOT ASSUME THE FRAME (run #22, 2026-08-24)

Run #21 measured a 55% overstatement on one wave and generalised it to "every weekly net one-way figure is an upper bound". Run #22 re-netted the **July episode** (Jul 6-27) feed-to-drain as one window:

| Frame | Net one-way | vs sum of weekly nets |
|---|---|---|
| July episode Jul 6-27, one window | **829,863** | 833,754 summed -> **0.47% apart** |
| August wave Aug 3-24, one window | **392,029** | 579,032 summed -> **47.7% overstatement** |

Weekly framing was **accurate** for July. The overstatement is a property of waves whose feed and drain legs sit in **different reporting weeks** - which the August wave does (UPbit fed 319,000 in one week and took 130,000 back in the next) and July's did not.

**Rules**:
1. Report the wave-window net alongside the weekly figure **when a straddle is detected**, not unconditionally.
2. The diagnostic tell is unchanged: a weekly circularity reading far outside the 63-80% band means a return leg landed whose feed sat in a prior window.
3. Do not generalise a measurement correction from a single instance. Run #21 did, and run #22 had to correct the correction.

### DO NOT PUBLISH A CORRECTED DIRECT-NODE FIGURE FROM A PARTIAL UNBONDING SUBTRACTION (run #22, 2026-08-24)

Run #21 subtracted ONE wallet's 229,865 EGLD from the staked-minus-delegated residual and published the remainder as "direct-node stake GREW +52,848". That subtraction is incomplete by construction - delegator unDelegations happen across the whole provider set every week. Run #22 measured **70,498 EGLD across the ten largest-moving providers alone**, plus 69,583 from p2p_org_'s node unstake, against a residual of 114,046.

**Rule**: when measured unbonding in flight meets or exceeds the residual, the correct statement is that **the residual carries no extractable direct-node signal** - not a smaller direct-node number. Publish `corrected_direct_node_egld: null`. Run #21's +52,848 is withdrawn on this basis.

### RECOVER NULL TOKEN PRICES FROM THE LIST ENDPOINTS BEFORE DERIVING THEM (run #22, 2026-08-24)

`/tokens/{id}` and `/tokens?sort=marketCap` can disagree about whether a price exists **for the same token in the same run**. Run #22: `/tokens/SWTAO-356a25` returned `price: null` through the main pass and all four isolated 2.5s retries (the run #14 pattern that previously forced a prior-price carry-forward), while the list endpoint carried a live $305.80 and a populated marketCap.

**Fallback order**, most-preferred first:
1. Cross-check the `/tokens?sort=...` list endpoints already in the snapshot. A measured value beats a derived one and costs one lookup.
2. Run #11 accumulator ratio (`swtao_price = wtao_price * prev_swtao/prev_wtao`).
3. Run #14 prior-price carry-forward, marked `_price_estimated`.

Write whatever is recovered back into the persisted snapshot so `audit_report.py` and the report stay self-consistent.

### WHEN TWO INSTRUMENTS FOR THE SAME QUANTITY DISAGREE, ADJUDICATE (run #22, 2026-08-24)

Run #22's two demand instruments inverted against each other: DEX turnover tripled to 10.83% of pool TVL/day while the identifiable bid read **exactly zero** (the Mega Whale absorber's balance unchanged to four decimal places, no transactions).

Reporting that as a contradiction is a non-answer. The adjudication: **turnover covers a whole venue and is price-independent; the identifiable bid is one wallet.** On this evidence the single-wallet proxy has stopped being informative about aggregate demand and should be repaired (discover absorbers dynamically from the desks' outbound terminals) or retired.

**Rule**: when two instruments for one quantity disagree, say which is better measured and why, in the same paragraph. Do not present both and leave the reader to choose.

### COMPUTE WHALE TIERS ON A COMMON-ADDRESS BASIS (run #22, 2026-08-24)

`previous.json` stores the top 60 accounts; the collector pulls the top 100. Bucketing both raw produced a phantom **+264,556 (+18.8%) mid-tier gain** that is entirely the extra 40 stored addresses. Restricting to the addresses present in BOTH snapshots (48 this run) gives +19,840 (+1.4%).

This is the run #18 "exclude newly tracked addresses from their first WoW delta" rule applied to the tier aggregates. It compounds with the run #14 boundary-crossing guard: check both before narrating a tier move.


### A REQUIRED FIELD THAT IS PRESENT-BUT-NULL IS AS FATAL AS A MISSING ONE (run #22, 2026-08-24)

Run #22 published `staking_intelligence.unbonding_in_flight.corrected_direct_node_egld: null`. That was the **correct editorial call** - the residual was fully absorbed by unbonding in flight, so no direct-node number could honestly be published. Both pre-publish gates exited 0. The deployed page was **blank**: `formatEgldBare(null)` calls `null.toLocaleString()`, and with no App-level error boundary the whole React tree unmounted. HTTP 200, no spinner, no message - the run #8 failure mode, reached by a different route.

The validator's dashboard-invariant check only verified that required fields were **present**, and only rejected nulls in a hand-listed set of STRING fields. Presence is not sufficient.

**Fixed in the same run**:
- `scripts/validate_report.py` now rejects ANY present-but-null required field, unless its full path is listed in a new `NULLABLE_REQUIRED` set.
- An entry may only be added to `NULLABLE_REQUIRED` **after** the corresponding component gains a null branch. The comment in the file says so; keep it that way.
- `UnbondingCard` renders "no signal extractable" for the null case and the TS type is `number | null`.

**The wider lesson, and the standing gap**: schema validation cannot prove a page renders. Twenty-two runs of it did not catch this. The publish gate should end with a headless load of the live URL that fails on any console error - a few seconds, and the only check that exercises what the reader actually sees.


### RUN A NEWLY INVENTED DETECTOR BACKWARDS OVER THE ARCHIVE, IN THE SAME RUN (run #23, 2026-08-31)

Run #22 invented the deregistration signature (`locked == 0` with `numNodes > 0`), applied it to one week, and published "the first operator deregistration in tracking". Applying that same signature to the stored snapshots - one loop over `data/collected/*.json` - shows it was wrong by two cases and ten runs:

| Provider | State | Discovered |
|---|---|---|
| ledgerbyfigment | 170,808 EGLD -> 0 between the 2026-06-08 and 2026-06-15 snapshots (run #13 window), 7 nodes and 3,961 delegators retained | run #23, retrospectively |
| stakedinc | zero-locked with 10 nodes and ~639 users across the ENTIRE stored archive | run #23, retrospectively |
| p2p_org_ | 69,583 -> 0 in the run #22 window; exit COMPLETED run #23 (removeNodes x11 + unBondNodes, numNodes 50 -> 0) | run #22 |

**Why both were invisible**: `previous.json` stored only providers with `locked > 0`. A provider that goes to zero drops out of the stored comparison set entirely and can never register as a WoW event. Fixed run #23 - the snapshot now stores the full `/providers` list (187 entries including zero-locked).

**Rules**:
1. Any claim of the form "first X in tracking" must be checked against the archive with the detector that found it, in the same run. The archive is where a detector's false-negative rate is measurable and the check is nearly free.
2. Widen the signature to `locked == 0 AND (numNodes > 0 OR numUsers > 0)` - a fully exited operator removes its nodes and stops matching run #22's version.
3. Store the full provider list, not a filtered subset. Any filter applied at snapshot time is a class of event the model can never see.

### PARTICIPATION INERTIA IS A MEASURED CONSTANT, NOT A WEEKLY OBSERVATION (run #23, 2026-08-31)

The deregistration backfill turned a one-week question into three natural experiments running at different durations, all with the same answer:

| Contract | Weeks at 0% APR | Delegators | Change |
|---|---|---|---|
| ledgerbyfigment | 11 | 3,961 -> 3,883 | -2.0% |
| stakedinc | whole archive (13+) | 639 -> 637 | -0.3% |
| p2p_org_ | 2 | 1,244 | 2 unDelegate calls in a week (0.16%) |

Against that, the two 100%-fee providers' BOOKS collapsed - egldstakingprovider -51.8% over three weeks, procryptostaking -20.5% over two - with user counts down 7.2% and 1.3%.

**Rule**: state it as a rate, not as a surprise. In this delegation market a total loss of yield removes roughly half a provider's EGLD in a month and roughly two percent of its people in a quarter. Capital responds to price signals; people do not. Note also that four wallets called `reDelegateRewards` on p2p_org_ - a contract with zero stake and zero APR - in the run #23 window, so the inertia is active rather than merely passive.

### THE PRE-FLIGHT ADDRESS VALIDATOR DOES NOT COVER COLLECTOR SOURCE (run #23, 2026-08-31)

`scripts/validate_addresses.py` checks `known-addresses.json` and `previous.json`. Run #23 hardcoded a Binance hot wallet with an invalid bech32 checksum directly in `collect_run23.py`. It returned HTTP 400, `paged_txs` broke on the error dict and returned `[]`, and the collector printed "0 outbound recipients >=10K" - which, taken at face value, would have resolved a pre-committed test on evidence that did not exist. The valid address is `erd1sdslvlxvfnnflzj42l8czrcngq3xjjzkjp3rgul4ttk6hntr4qdsv6sets`; the invalid variant ends `gdvvz2eqra76xc`.

**Rule**: extend the pre-flight to grep `erd1` literals out of `scripts/*.py` and validate those too. This is the third time in six runs an invalid address has produced a confident wrong answer rather than an error (runs #12, #18, #23).

### A PAGINATION HELPER MUST NOT RETURN [] ON AN ERROR (run #23, 2026-08-31)

`paged_txs` used `if not isinstance(batch, list) or not batch: break`, which treats an HTTP error dict identically to a genuinely empty page. Three separate findings were nulled this way in run #23 - the Binance hot-wallet scan (HTTP 400), the unbond wallet's info/delegation/outbound (HTTP 429) and the entire wave-window desk trace (transient). Every one produced "nothing happened" rather than a failure.

**Rules**:
1. Distinguish "no transactions" from "the query failed" and record the latter in the failure list.
2. Use exponential backoff on 429 (1s doubling to ~12s), not a fixed delay. Run #23's main pass at a fixed 0.22s drew 429s once the all-provider scan pushed it past roughly 800 requests; the followup pass with backoff hit zero errors.
3. A run that adds a large new scan should re-check its total request count against the rate limit, not just its per-request delay.

### SPECIFY A TEST'S THRESHOLD ON THE QUANTITY THAT CARRIES THE SIGNAL (run #23, 2026-08-31)

Run #22 registered: "the Binance.com hot wallet falling more than ~150,000 next week while the OTC feed continues = the two channels are one programme". The hot wallet's balance ROSE 159,709 - because custody refilled it with 500,000 faster than it distributed 135,003 into the desks. Applied literally the test would have resolved backwards.

**Rule**: a balance-delta threshold cannot detect a flow whose funding outruns its spending. Write flow tests on the flow (does X send to Y, and how much), which is directly measurable via the two-hop trace the model already runs.

### BRANCH CONDITIONS MUST PARTITION THE OUTCOME SPACE (run #23, 2026-08-31)

Run #22's compound-rate test offered "below 56%" and "above 59%". The reading was 57.51% and the test could not resolve. That is the second consecutive run in which a test failed on how it was WRITTEN rather than on what happened - run #22's unbond test lacked a no-action branch, this one lacks the middle of its range.

**Rule**: branches must cover every possible outcome with no gaps. Where a middle band is genuinely uninformative, say so explicitly as a third branch ("between X and Y = no conclusion, re-measure") rather than leaving it undefined.

### REPORT VENUE THROUGHPUT IN EGLD AS WELL AS USD (run #23, 2026-08-31)

Run #22 flagged that the DEX turnover spike was confounded with a +30% price move. One extra division settles it: in run #23 the USD turnover ratio held at 10.51% (above its 5% regime threshold) while EGLD-denominated volume FELL from 63,885 to 61,110 EGLD/day (-4.3%) and EGLD pool depth fell 1.4%. Both sides of the ratio scale with price, so the ratio is nearly price-invariant while its components are not - and the EGLD series is the one that answers a chain-native demand question.

**Rule**: record `volume_24h_egld` and `pool_tvl_egld` alongside the USD figures every run, and treat the EGLD series as primary for demand.

### THE DESKS' OUTBOUND TERMINALS CONTAIN NO ABSORBERS (run #23, 2026-08-31)

Run #22 proposed repairing the identifiable-bid instrument by discovering absorber wallets dynamically from the desks' outbound terminals instead of watching one Mega Whale. All 14 terminals were scanned for retention: **505,597 EGLD passed through and 8,545 (1.7%) stayed**. Every terminal is a zero-balance, high-nonce pass-through router.

**Conclusion**: there are no absorbers on the outbound side of this pipeline to discover - it delivers exclusively to venues through routers. The single-wallet instrument is RETIRED rather than repaired, and the demand read is carried by DEX turnover (in EGLD terms) plus ex-pipeline withdrawal breadth. A decisive negative is a better outcome than a plausible-looking replacement.

### THE PIPELINE CAN ACCUMULATE FASTER THAN IT DISTRIBUTES (run #23, 2026-08-31)

Every prior wave drained inventory while delivering. Run #23 did both at once: the desks pushed **272,396 EGLD one-way** to five venues (Binance.com, Bybit, Gate.io, Bitget - new - and Unknown Whale I) and still ended **+156,356 at a record 266,213**, 2.4x the prior record and more than the entire run #17 peak wave delivered (409,680 from a starting inventory of 335,430).

**Rule**: report desk INVENTORY (a stock) alongside gross throughput and net one-way (flows). The stock is what tells a reader whether a distribution wave is ahead of them or behind them, and no flow figure contains it. Added to `previous.json` as `otc_desk_inventory_series` and to the baselines as `otc_desk_inventory_egld`.

### BINANCE IS NOW A DESK FEEDER, NOT JUST THE PIPELINE'S DISTANT ORIGIN (run #23, 2026-08-31)

Run #10 traced the pipeline's ultimate origin to Binance through a source-funder chain. Run #23 shows direct participation: the staking custody sent 500,000 to the Binance.com hot wallet, which forwarded **135,003 EGLD** into the OTC desks - 73,421 through the feeder labelled in run #19, 30,554 through the run #20 feeder, 21,297 direct - and returned 303,242 to custody. Custody has fallen 497K over two weeks.
...
### A 404 ON A HARDCODED IDENTIFIER IS A SPELLING FAILURE UNTIL PROVEN OTHERWISE (run #23, 2026-08-31, post-publication)

Run #23 published `/tokens/WTAO-3ec9c0: HTTP 404` in its failure list and concluded "the run #11 accumulator-ratio fallback for SWTAO is permanently unavailable". Re-querying after publication shows that is wrong twice over.

`WTAO-3ec9c0` does not exist and never did in the stored archive — it has returned HTTP 404 on **every run since #13**, eleven consecutive snapshots. One call to `/tokens?search=WTAO` returns the live token: **`WTAO-4f5363`**, WrappedTAO, $221.90, 1,177 holders.

**Consequences that were mis-attributed for eleven runs**:
- The run #11 accumulator fallback (`swtao_price = wtao_price * prev_swtao/prev_wtao`) needs a WTAO price. It has been unavailable the whole time.
- Run #14 recorded that the fallback "was ALSO unavailable because WTAO price was null too" and blamed the dataApi feed. Withdrawn: the price was never requested for a token that exists.

**Rules**:
1. Search the endpoint for the ticker before concluding a source is gone. One call.
2. A source that has failed EVERY run since it was added never worked — that is a bug in our code, not an outage. Only a source that used to succeed has actually broken.
3. Re-test the standing failure list each run. A failure that repeats weekly stops reading as news, which is precisely how this survived eleven runs of being carried forward as a known gap.

### SEPARATE RECOVERED SOURCES FROM MISSING ONES (run #23, 2026-08-31, post-publication)

Run #23's metadata listed four failed sources, three of which were transient failures of the main pass that the follow-up pass re-queried successfully. Those three were simultaneously present in `data_sources_ok` (as follow-up entries) and in `data_sources_failed` (as main-pass failures), so the dashboard header counted them and told the reader **"4 failed"** when exactly one thing was missing from the report.

`metadata.data_sources_recovered` now holds sources that failed on the main pass and succeeded on retry. The header reads **ok / retried / missing**. The audit trail is preserved — a retry is still recorded — without overstating the holes in the data.

**Rule**: a source is only "failed" if the report ships without its data. If a follow-up pass recovered it, it is recovered.


### DISCOVERING LIQUID-STAKING PROTOCOLS WE DO NOT TRACK YET (run #23, 2026-08-31)

Until now the tracked LSD set was whatever had been added by hand: Hatom and XOXNO, plus a labelled-but-unmeasured JewelSwap. There was no way to notice a new one. A protocol could reach hundreds of holders before anyone looked, and one had.

**The signature.** A liquid-staking protocol is a smart contract that (a) owns an ESDT and (b) has `userActiveStake > 0` on delegation contracts. It stakes EGLD for other people and hands them a transferable claim; nothing else on this chain has that shape.

**The approach that does NOT work.** Scanning delegation contracts for smart-contract senders calling `delegate` returns *nothing*. A contract-to-contract delegate call settles as a smart-contract result and never appears in `/accounts/{provider}/transactions`. Verified against the VoxEGLD contract: zero hits as a sender in the provider's `/transactions`, **50** in its `/transfers`. This is the run #19/#21 lesson (SC results are invisible to value-transfer scans) surfacing in a new place, and it cost a full scan of 35 providers to learn.

**The approach that works.** `/tokens/{identifier}` exposes `owner` directly, so the chain is token → owner → delegation, one hop:

1. Gather candidate tokens: `/tokens?search=EGLD|staked|stake` plus three pages each of `sort=marketCap|accounts|transactions`. The ranked lists catch anything with adoption; the name search catches a launch with neither market cap nor holders yet, which is precisely the case that would otherwise go unseen for months.
2. Reduce to distinct owners where `owner` starts with `erd1qqqq`. 333 tokens collapsed to 48 owners.
3. For each owner, `/accounts/{owner}/delegation`. Any `userActiveStake > 0` is a liquid-staking protocol.

About 60 calls total. `scripts/discover_liquid_staking.py`.

**First sweep — three real protocols were being missed**, and the largest was not the one that prompted the search:

| Protocol | Token | Staked EGLD | Holders | Status |
|---|---|---|---|---|
| Hatom Liquid Staking | SEGLD-3ad2d0 | 845,814 | — | tracked |
| XOXNO EGLD Liquid Staking | XEGLD-e413ed | 256,493 | — | tracked |
| **SALSA** (Staking Agency) | LEGLD-d74da9 | **6,451** | 382 | **was untracked** |
| JewelSwap Liquid Staking | JWLEGLD-023462 | 1,795 | — | labelled, never measured |
| **Dinovox VoxEGLD** | VOXEGLD-5872e5 | **1,757** | 78 | **was untracked** |
| **VestaX Finance** | VEGLD-2b9319 | **1,121** | 439 | **was untracked** |

Two further contracts matched at 1–2 EGLD with 2 holders each — abandoned test deploys, excluded by a 100 EGLD floor.

**Rules**:
1. Run the sweep every week. It is ~60 calls and it is the only mechanism that can surface a protocol nobody told us about.
2. Apply a 100 EGLD staked floor. Below that it is a test deploy, not a protocol.
3. Report emerging LSDs on the **supply** basis like the established ones (run #13 rule). Supply is the leading indicator, and SALSA had reached 5,080 LEGLD before it was measured once.
4. The same signature generalises: swap the delegation check for another balance-sheet test and it finds any contract issuing a receipt against a position.



### COMPARE ACROSS A SNAPSHOT-SCHEMA CHANGE ON A COMMON BASIS (run #24, 2026-09-07)

Run #23 fixed a real bug by storing the FULL `/providers` list in `previous.json` instead of the `locked > 0` subset. The next run's naive delegator comparison - current `locked > 0` total against the stored sum - printed **-28,141 delegators**, a 14% collapse that is entirely the schema change. The real number is **-73**.

Rule: when the shape of a stored snapshot changes, every WoW derived from it must be recomputed on the OLD basis, or the change itself becomes the finding. The assembler now filters `previous.json` to `locked > 0` explicitly and stores the full-list total separately as `prev_users_all_contracts`, so the two bases are never confused again.

### A DETECTION SIGNATURE NEEDS A RECENCY QUALIFIER (run #24, 2026-09-07)

Run #22 detected deregistration as `locked == 0 with numNodes > 0`; run #23 widened it to `locked == 0 AND (numNodes > 0 OR numUsers > 0)` because a fully exited operator has no nodes left. Applied to the full stored list, the widened signature matches **82 contracts carrying 28,032 delegator records** - and running it backwards over all 16 snapshots shows only **four** ever had `locked > 0` inside the archive. Exactly one of the 82 saw a single inbound transaction in the week.

Rule: a state predicate describes a state, not an event. Deregistration is an observed TRANSITION from `locked > 0` to zero in a stored snapshot. Everything else is archaeology - report it as cohort context, never as this week's news, and score the fourth-deregistration test on transitions only.

### NEVER SUBTRACT ONE WINDOW'S FLOW FROM ANOTHER'S (run #24, 2026-09-07)

The run #23 absorber scan set `received_from_desks = max(7-day, wave-window)` and `forwarded_out = 7-day only`, then reported `retained = received - forwarded`. In run #23 that happened to give the right conclusion; this week it produced **789,331 EGLD of apparent retention on terminals whose balances total 15,534**.

Rule: any derived quantity that mixes windows is wrong by construction, and a coincidentally-correct conclusion hides it. Retention is measured on the terminal's END-OF-WEEK BALANCE. Where two windows must appear in one figure, name the window in the field name.

### SPECIFY A ROUTING TEST ON WHAT A WALLET SENDS, NOT ON WHAT IT HOLDS (run #24, 2026-09-07)

Three runs asked whether Binance funds the OTC desks. Run #22 wrote the threshold on the hot wallet's BALANCE and could not resolve; run #23 re-specified it on the FLOW (hot -> desks or known feeders) and it resolved on the first attempt at **97,390 EGLD** against a 50,000 bar. The balance test would have read backwards this week: the hot complex fell while custody ROSE +372,434, and the feed ran anyway.

Rule: an intermediary's balance is a residual of two flows and can move either way while the flow under test is constant. Test the flow. The corollary fired too - the registered reading ("a standing programme means the custody drawdown is scheduled supply") was itself wrong, because custody refilled; report the branch that fired AND the part of its reading the data contradicts.

### A TRACKED CONTRACT READING ZERO WHILE ITS NEIGHBOURS DO NOT IS AN ADDRESS PROBLEM (run #24, 2026-09-07)

The JEXchange aggregator returned 0 transfers for four consecutive runs while the JEXchange fees contract reported thousands. Tracing the fees contract's INBOUND leg back to its callers (the run #21 rule) identified the live router in one query: **11,802 transfers in 7 days**, with four more routers behind it. Same failure mode as `WTAO-3ec9c0`, which 404'd for eleven runs.

Rule: when a tracked contract reads zero and a related contract does not, suspect the identifier before the protocol, and re-derive from whichever neighbour still reports. Both of this run's DeFi corrections came from this one habit.

### REJECTING A CANDIDATE REGIME SHIFT IS A RESULT (run #24, 2026-09-07)

Run #23 flagged record desk inventory as a CANDIDATE regime shift and declined to promote it, because the methodology requires a new level to hold two weeks. It fell 64% the following week. The rule prevented a one-week staging peak from being published as a structural change in the pipeline's operating scale.

Rule: keep the two-week promotion requirement, and report the rejection explicitly in `trend_indicators.regime_shifts` so the archive records that the test ran.

### SLICE A BUSY WALLET BY DAY, AND CHECK THE CAP LOG AGAINST THE RE-SCAN LIST (run #24, 2026-09-07, post-publication)

Two of the three sources run #24 first published as failures were not what they looked like.

**The Binance.com hot wallet was genuinely truncated** - it fills a 1,000-transaction budget in 4.4 of 7 days - and the fix is to page it one calendar day at a time, deduping by `txHash`, so each slice fits the budget. That took the hot-to-pipeline figure from 112,126 to **255,442 EGLD**, a 2.3x undercount on a number that resolves a pre-committed test. Rule: any address above roughly 200 transactions a day is queried in daily slices, not in one window.

**The 16 page-capped provider scans were not truncated at all.** They were the shallow first pass, and every one was re-paged at 30 pages in the same run - largest deep result 765 txs against a 1,500 cap. Rule: a page-cap log entry is not a data gap if a later pass superseded it. Reconcile the cap log against the re-scan list before publishing either.

And a third, smaller trap: **attribution shifts if the label set changes between assembly passes.** Re-running the assembler after `update_data` added 17 router labels moved 110,573 EGLD out of "unattributed" and into named feeders. Same data, better labels. Freeze the label set for a run, or re-run every stage after a known-addresses change - never mix stages across it.


### READ VENUE STATE FROM THE CONTRACT, NOT FROM API ABSENCE (run #25, 2026-09-14)

`/mex/economics` returned MEX at `price: 0`, and the MEX/WEGLD pair (last week the #2 deepest pool, $442,977) had vanished from `/mex/pairs`, with `/mex/pairs/MEX-455c57/WEGLD-bd4d79` returning 404. Read as an outage, that would have gone into the failure list. The pair contract `erd1qqqqqqqqqqqqqpgqa0fsfshnff4n76jhcye6k7uvd7qacsq42jpsp6shh2` still exists. Its own transaction list shows a `pause` call at 2026-09-13 16:43 UTC from the router owner `erd1ss6u80ruas2p...`, followed by 172 failed transactions (116 `removeLiquidity`). Reserves are 191B MEX + 141,281 WEGLD.

**Rules**:
1. When a tracked pair or contract disappears from an aggregate endpoint, query the contract directly: `/accounts/{addr}`, `/accounts/{addr}/tokens`, and `/accounts/{addr}/transactions/count?function=pause|resume|setState...`.
2. The xExchange API drops paused pairs and zeroes MEX price. Every MEX-dependent figure then needs a like-for-like base: remove the pair from the PRIOR pool TVL and volume, and price MEX from a third party (CoinGecko `maiar-dex`).
3. An H-token whose underlying has no on-chain price must be excluded from lending TVL. HMEX alone added +44pp to Hatom Lending in EGLD terms.

### PROVIDER IDENTITIES ARE MUTABLE - JOIN ON THE CONTRACT ADDRESS (run #25, 2026-09-14)

Run #22 moved the provider join to `identity or provider` to fix an address/identity mismatch. Run #25 hit the reverse failure: `cslabsio` renamed to `chainstatelabs`, and the identity-keyed join booked 177,587 EGLD as a leaver. The real move was +200. `kevinlallement` -> `dinovox` did the same at 8,804 EGLD. `previous.json` stores `address` on every provider row, and that address is the stable key. **Rule**: join on `address`, log identity changes as an event list, and treat any "left the active set" entry whose address is still present as a rename.

### PUT PIPELINE FLOWS IN MARKET SCALE (run #25, 2026-09-14) - closes run #24's top recommendation

CoinGecko `/coins/elrond-erd-2/tickers?depth=true` gives per-venue `cost_to_move_up_usd` / `cost_to_move_down_usd` (±2% depth) and `/market_chart?interval=daily` gives rolling 24h spot volume. The first reading reframed the report. Run #24's record 536,459 EGLD delivery was **2.1%** of that week's CEX spot volume, and run #25's 401,353 was **2.3%**. Weekly net delivery into Binance+Bybit (~$1.23M) was about 10x their combined ±2% bid depth ($125K), so delivery is worked into the book over days.

**Rules**:
1. Report net one-way delivery as a share of 7d spot volume every week, alongside the absolute figure.
2. Do not read a week's delivery against the week's price as if nothing else traded. The open question is whether a persistent, one-directional ~2% offer moves EGLD relative to BTC, and that is the registered test.
3. This is third-party data. Store it in the snapshot (`cg_tickers`, `cg_market_chart`) so the series is reproducible.

### THE BILATERAL INVERSE RULE IS PARTLY MECHANICAL - READ IT ON THE HEGLD LEG (run #25, 2026-09-14)

Hatom Lending TVL is summed across dollar, BTC, ETH and EGLD markets and then divided by the EGLD price. When EGLD falls, the non-EGLD collateral rises in EGLD terms with no depositor doing anything. Run #25 had the "correct" inverse sign (ex-HMEX +8.35% on a -7.59% week) while EGLD depositors actually withdrew: HEGLD supply -5.45%, and the EGLD money-market contract lost 89,886 EGLD. **Rule**: the behavioural test of the rule is HEGLD supply (EGLD-native deposits). Report the aggregate ratio as context only.

### CONCURRENT SCRIPTS SHARE ONE RATE LIMIT; DISCOVERY MUST SEED FROM LAST WEEK (run #25, 2026-09-14)

Running `delegator_behavior.py` alongside the collector produced HTTP 429 on the OTC Distribution Wallet's wave scan (inbound 0, outbound cut at 400). `paged_txs` logged these as errors, so `followup_run25.py` could recover them. It also cut the reward sample to 4 of 8 providers with nothing in its output to say so, and that sample produced a false 46.79% compound rate. Separately, `discover_liquid_staking.py` failed to surface SALSA (6,420 EGLD) and VestaX (1,121) even though both still stake, because its candidate-token pages vary week to week.

**Rules**: run API-heavy scripts sequentially; check `providers_sampled` before using the compound rate; seed the LSD sweep from `previous.json` `lsd_protocol_seeds`.


### PLOT THE TIMESTAMPS BEFORE NARRATING A SEQUENCE (run #25, 2026-09-14)

The first draft of run #25 said xExchange paused the MEX/WEGLD pool and MEX "went up sevenfold the next day". That ordering came from CoinGecko DAILY closes (Sep 13: 4.08e-07, Sep 14: 3.05e-06). Drawing the hourly series against the pause timestamp for the dashboard showed the reverse: MEX was 1.7x by 15:00 UTC and about 4x by 16:00, and the pause landed at 16:43, inside the move. A daily close stamped at 00:00 cannot tell you whether a move came before or after an event at 16:43 on the previous day.
...
### CHECK CHAIN LIVENESS BEFORE ANYTHING ELSE (run #26, 2026-09-21)

MultiversX stopped producing blocks at 2026-09-19 07:37-07:41 UTC (shard 0 07:37:49, metachain 07:41:12) after a VM-level exploit. The run #26 collector ran 55 hours later and the API answered every call normally: /economics, /accounts, /providers and /stats all returned plausible data. Nothing said the chain had stopped. It was found only because /stats `blocks` did not change between two calls an hour apart, and `/blocks?size=1&shard=N` then showed the last block's timestamp.

**Consequences if missed**: every "7-day" window silently becomes 5.3 days; every 24h aggregate (/mex/pairs volume24h, /transfers/count after now-24h) reads zero and looks like a collapse; and the week's price move is compared against flows that stopped two days earlier.

**Rules**:
1. Read the latest block timestamp per shard at collector start. If any shard is more than an hour behind, record the halt time and end every flow window there.
2. A metric whose window contains no blocks is NOT EVALUABLE. Report it as such, and do not append it to a baseline (a zero would poison the z-score for eight weeks).
3. Post-halt market readings (CoinGecko order book, spot volume) are taken while exchange deposits and withdrawals are closed. Keep them as a flagged side-reading, not as the next point of the series.

### CHECK ACCOUNT BALANCES AGAINST TOTAL SUPPLY (run #26, 2026-09-21)

After the exploit, `/accounts?sort=balance` listed a wallet holding 41,307,412 EGLD and its contract 25,559,887, more than the 30,816,932 total supply that /economics still reported. The derived stage printed an exchange net flow of +4,913,831 EGLD, ten times the largest week in tracking. 4,859,875 of it was minted EGLD deposited at Binance.com, KuCoin and MEXC in three minutes.

**Rules**:
1. Any account above total supply, or a NEW top account above 5% of supply, is invalid state until explained. `audit_report.py` check 9 now errors on it unless the report documents the incident (`whale_intelligence.chain_halt_incident`).
2. Net incident balances out of every balance-derived metric: exclude the attacker's addresses from the top-account set, subtract what the fan-out delivered from each receiving wallet, and publish the raw figure alongside. Store the NET balances in previous.json, so a recovery that reverses the invalid state does not register next week as a phantom outflow.
3. Trace the fan-out one hop past the fresh wallets before writing. That single pass quantified exactly how much reached each venue.

### AN ATTACKER'S CASH-OUT ROUTE LABELS EXCHANGE WALLETS (run #26, 2026-09-21)

The exploit wallet sent minted EGLD through 12 fresh one-hop wallets to Binance.com, KuCoin, MEXC, an unlabelled high-nonce wallet (erd1a6lte0...u48u4l) and Unknown Whale I. Someone cashing out sends to exchange deposit infrastructure. Unknown Whale I had been treated since run #19 as an OTC operator's inventory, on both legs of the desks; this is strong evidence it is an exchange hot wallet. The claim is withdrawn and registered as a test (>500 distinct inbound senders in 7 days = exchange).

**Rule**: a wallet on the far side of a known cash-out path is a deposit address until shown otherwise. Treat long-standing "operator" labels that rest on flow shape alone as provisional.

### A PAUSED POOL CAN STILL TRADE FOR WHITELISTED CALLERS - READ ITS /transfers (run #26, 2026-09-21)

Run #25 read the MEX/WEGLD pause from the pair's transactions. Its /transactions list for the pause window then showed only the admin's whitelist calls. The pair's `/transfers` showed 89 swaps on Sep 15 19:15-21:11 from a contract Hatom's deployer had created that afternoon. It redeemed 13.04T HMEX of seized collateral for 270B MEX and sold 222B of it into the paused pool for 75,792 WEGLD, while every other caller's swap or withdrawal failed. The router owner had added each MEX pool to its pause whitelist at 15:56. Hatom's EGLD money market went 28,418 -> 112,295 EGLD.

**Rule**: when a contract is paused, enumerate `addToPauseWhitelist`-type calls on the admin and read the contract's /transfers (smart-contract results), not only its /transactions. The privileged exit is the part a protocol's statement is least likely to mention.

### DECODE WITHDRAW AMOUNTS FROM SMART-CONTRACT RESULTS (run #26, 2026-09-21) - closes run #25 rec #5

`withdraw` on a delegation contract carries no amount argument. `/transactions/{hash}?withScResults=true` returns the EGLD sent back to the caller in `results` (receiver == caller). 377 calls decoded in about 90 seconds: 108,941 EGLD returned in the 5.3 pre-halt days, against 84,169 EGLD of new unDelegations. Report both every run. The unbonding queue's net direction is the difference, and before this run it was inferred from call counts.

### ADDRESS POISONING OF PIPELINE ROUTERS (run #26, 2026-09-21)

Two wallets whose first and last characters match the OTC desks (erd1z7wy...ppm63r vs the OTC Distribution Wallet erd1z7fn...nm63r; erd1v6k4...ryxnk5 vs the UPbit OTC Desk erd1v6x9...hcxnk5) sent 369 dust transfers of 0.0001 EGLD to 33 pipeline routers each in 30 days. They show up in hub traces as "UNRESOLVED" terminals at dust amounts. `attribute()` weights terminals by value, so they do not move venue attribution. Watch them for any inbound above dust, which would mean a router operator was fooled.
...
| 22 | 2026-08-24 | **A +30.43% RALLY ARRIVED WITH DISTRIBUTION, AND THE FIRST OPERATOR DEREGISTRATION IN TRACKING.** EGLD $2.76 -> $3.60, the largest weekly move in twenty-two runs, but a BETA move (BTC +23.78%, ETH +30.67%; EGLD between them) with no EGLD-specific component. **The OTC pipeline restarted into it**: UPbit fed 297,000 EGLD against 14,000 the week before, gross throughput 767,206 (3.4x), net one-way 252,109 at 67% circularity (back inside the 63-80% band), and the desks RELOADED +49,292 to a record 109,857 - more loaded than delivered. Destinations resolve two hops to Bybit +132,402, Binance.com +81,652, Gate.io +28,346, with UPbit the sole net source: supply arriving at order books. Run #21's feed-resumption test fired on the standing-programme branch at ~2x its threshold, a week early. **Binance's staking custody drew down -300,000 straight into its hot wallet** in one traceable transfer - the bearish branch pre-registered since run #9, firing for the second time (run #15 was the first). **p2p_org_ called unStakeNodes**, taking 67,500 stake + 2,083 topUp to ZERO locked while keeping 50 listed nodes, 1,244 delegators and an APR of 0 -> new rule: a deregistered provider does NOT leave /providers; detect `locked == 0 with numNodes > 0`, and net it out of BOTH delegation TVL (87% of this week's -80,367) and the delegator count (1,244 of an apparent -1,311 'break' in a nine-week flat series; ex-p2p_org_ the base moved -67). **The 229,865 EGLD unbond did not move** - both legs completed or nearly completed unbonding and NEITHER was withdrawn -> new rule: destination tests need a no-action branch with a time bound, or a counterparty doing nothing produces an unresolvable test. **Run #21's blanket weekly-netting rule is NARROWED**: the July episode (Jul 6-27) re-netted feed-to-drain gives 829,863 against 833,754 summed weekly - 0.47% apart, so weekly framing was ADEQUATE there; the August wave (Aug 3-24) still overstates 47.7% because it straddles a boundary -> test the wave, do not assume the frame. **Run #21's '+52,848 corrected direct-node' is WITHDRAWN**: 70,498 EGLD of fresh unDelegations across the ten largest-moving providers ALONE, plus p2p_org_'s 69,583, exceed the 114,046 residual -> publish null, not a smaller number. **Demand instruments inverted against each other**: DEX turnover TRIPLED to 10.83% (volume +328%, depth +15.6%, third consecutive rise, far past the 3.5% threshold) while the identifiable bid read exactly ZERO for a second week -> new rule: adjudicate, do not present both; turnover covers a venue and is price-independent, the bid is one wallet. **Every holding-side instrument de-risked into strength**: compound rate 57.03% (lowest of nine, third consecutive fall, institutional tier selling 3 of 4 claims - the first tier ever to lead with selling), Hatom Lending EGLD-TVL -8.39% (third up-week inverse confirmation at ratio 0.28, BELOW the 0.30 capacity-exhaustion threshold), USH burned -3.43% INTO the rally, and zero measurable LSD subscription. USDT recovered +0.44%, resolving run #21's -15.42% as one desk. MEX matched EGLD for a 5th week (+9.8pp cumulative) and BOTH candidate mechanisms were eliminated - supply -0.151% and pool depth grew with the market. **Silent-failure lesson**: the provider join used the wrong key (previous.json stores IDENTITY, the API stores ADDRESS), reporting 79 phantom 20K+ movers and zero unDelegate callers without erroring -> assert a match rate on every cross-snapshot join. Also new: SWTAO's price nulled on /tokens/{id} through four retries while the LIST endpoint had it live at $305.80 -> check the lists before deriving an estimate. Whale I's largest external counterparty resolved as a SECOND WALLET OF THE SAME OPERATOR, closing the classification. Withdrawal-breadth pipeline share 88.3% (was 47%), highest since run #17. Zero qualifying new tokens for a 6th week. 8/10 prior action items completed. **Caught post-deploy and fixed in-run**: both gates passed a report carrying `corrected_direct_node_egld: null` (the honest value) and the deployed dashboard rendered BLANK - `formatEgldBare(null).toLocaleString()` unmounted the React tree. The validator now rejects any present-but-null required field outside an explicit NULLABLE_REQUIRED allowlist, and the card handles the null case. Standing recommendation: end the publish gate with a headless render check, because schema validation has never proven a page renders. |
| 23 | 2026-08-31 | **RUN #22's HEADLINE WITHDRAWN, AND THE PIPELINE ACCUMULATED FASTER THAN IT DISTRIBUTED.** (1) Applying run #22's own deregistration signature BACKWARDS over the stored archive found TWO operator exits it had missed - ledgerbyfigment (170,808 EGLD, 3,961 delegators, run #13 window) and stakedinc (whole archive) - so "the first operator deregistration in tracking" is withdrawn; p2p_org_ was the third and completed its exit this run (removeNodes x11, numNodes 50 -> 0). Root cause: previous.json stored only locked>0 providers, so a provider going to zero could never register as an event; now stores the full 187-entry list. New rule: run a newly invented detector over the archive in the same run it is invented. (2) That backfill turns participation inertia into a MEASURED CONSTANT across three durations - ledgerbyfigment -2.0% of delegators over 11 weeks at 0% APR, stakedinc -0.3% across the archive, p2p_org_ 2 unDelegate calls from 1,244 users - against 100%-fee books falling 20-52%. Capital responds to price; people do not. (3) OTC: UPbit tranche 460,000 (series 14,000/297,000/460,000, 3x the escalation threshold), 272,396 net one-way delivered to FIVE venues (Bitget new), AND desk inventory ended +156,356 at a RECORD 266,213 - the first week the pipeline accumulated faster than it distributed. Wave #3 (Aug 17-31) nets 371,179 vs 524,505 summed weekly, a 41% overstatement predicted in advance by 62% circularity below the 63-80% band. (4) BINANCE IS A DESK FEEDER: custody -196,758 (2nd week, -497K over two) -> hot wallet -> 135,003 into the desks via the run #19 and #20 feeders. The custody watch and pipeline watch are one instrument. (5) The identifiable-bid REPAIR returned a decisive negative - all 14 desk terminals are zero-balance routers retaining 1.7% of 505,597 EGLD - so the instrument is RETIRED. (6) EGLD +6.39% to $3.83 while BTC -0.97% and ETH -1.07%, an EGLD-specific decoupling. (7) Three silent-failure defects found: an invalid hardcoded address in the collector (HTTP 400 -> empty list -> fabricated "zero"), a pagination helper that cannot distinguish error from empty, and HTTP 429s from exceeding the rate limit with the new all-provider scan. (8) First full-set unbonding scan: 151,443 EGLD from 300 callers across 107 contracts, 2.1x the ten-provider partial. (9) 8 of 8 pre-committed tests resolved, 7 as predicted (87.5%); the one inconclusive failed on non-contiguous branches - the second consecutive run to lose a test to a specification defect. | **(post-publication reconciliation)** Re-queried the four sources recorded as failed: three were transient and already recovered by the follow-up pass — they were double-counted as both failure and success, so the header said '4 failed' when one thing was missing; `metadata.data_sources_recovered` now separates them and the header reads ok / retried / missing. The fourth, `/tokens/WTAO-3ec9c0`, is a WRONG IDENTIFIER the collector has carried since run #13, 404ing on all eleven runs since; the live token is `WTAO-4f5363`. That withdraws run #23's own 'accumulator fallback permanently unavailable' claim and run #14's attribution of the same gap to the dataApi feed. **(liquid-staking discovery)** Built `scripts/discover_liquid_staking.py` after an operator flagged an untracked LSD. The delegation-side scan found nothing because SC-to-SC `delegate` calls are smart-contract results invisible to `/transactions`; the token-side chain (token -> `owner` -> `/delegation`) works in ~60 calls. First sweep found THREE untracked protocols - SALSA (LEGLD, 6,451 EGLD, 382 holders), Dinovox VoxEGLD (1,757 EGLD) and VestaX (VEGLD, 1,121 EGLD, 439 holders) - plus JewelSwap labelled but never measured. The largest miss was not the one reported.
| 24 | 2026-09-07 | **THE OVERHANG WAS DELIVERED, AND THE PRICE ROSE 17% THROUGH IT.** (1) Desk inventory 266,213 -> 96,114 (-64%) while the desks pushed a RECORD 536,459 EGLD one-way to order books (z=+4.60sigma, 31% past the run #17 peak of 409,680); wave #3 netted feed-to-drain across Aug 17 - Sep 7 is 878,809, more than double the largest wave previously tracked. Run #23's pre-committed drain test fires on its sub-120,000 branch, and its CANDIDATE regime shift on the same number is correctly REJECTED - the two-week promotion rule earned its keep. (2) EGLD +16.97% to $4.48 with BTC +0.36% and ETH +0.20%: a record delivery absorbed into a record up-week, and NO tracked demand instrument identifies the buyer (DEX volume -27.1% in EGLD, LSD supply flat a 7th week, staked ratio at an archive low, identifiable bid zero a 4th week, and 42% of the ex-pipeline withdrawal value is two wallets, one of them Binance hot-funded). Named as a gap in the model rather than filled with a story; the top recommendation is now to build an exchange-side demand instrument. (3) BINANCE'S FEED IS A STANDING PROGRAMME BUT NOT CUSTODY-FUNDED: hot wallet -> known OTC feeders 97,390 EGLD in a page-capped 4.4-day window (>50,000 branch) while staking custody REVERSED +372,434 after two weeks of -497K. The run #22 balance-based version of this test would have read backwards; the run #23 flow-based re-specification resolved on the first pass -> rule: specify a routing test on what a wallet SENDS, never on what it HOLDS. (4) THE DEREGISTRATION COHORT IS 82 CONTRACTS, NOT THREE: the widened signature matches 82 zero-stake contracts carrying 28,032 delegator records (14.0% of all records), but only FOUR have an observed transition inside the 16-snapshot archive and only ledgerbyfigment (-7 users, 47 inbound txs) shows any activity -> rule: a detection signature needs a recency qualifier; deregistration means an observed transition, not a zero reading today. (5) A STORAGE CHANGE ALMOST BECAME A HEADLINE: run #23's switch to storing the full /providers list makes the naive delegator WoW read -28,141, entirely artifact; the assembler now filters previous.json to locked>0 -> rule: recompute on a common basis across any snapshot-schema change. (6) THE ABSORBER RETENTION FIGURE WAS WRONG BY CONSTRUCTION for two runs (received = max(7d, wave) minus forwarded = 7d only); on the balance basis the 14 terminals hold 15,534 EGLD against 1,748,893 received -> rule: never subtract one window's flow from another's. (7) Two 100%-fee providers REVERSED their fees (100% -> 50% and 100% -> 35%, APR restored) and still lost book and users - restoring a yield does not bring capital back within a week. (8) The compound rate REVERSED to 62.05% (highest of eleven) in the week of the record delivery, resolving run #23's drift test on the 'stabilised' branch - the first of three attempts at that question to resolve on the data rather than on a specification defect. (9) THIRD consecutive contraction in the wrapped-dollar base (USDC -1.65%, USDT -2.39%) while USH MINTED +5.23%, reversing two burns: cash out, leverage in, and the run #23 claim that two burns made a behaviour is withdrawn. (10) JEXchange's aggregator address was STALE for four runs - the live router, re-derived from the fees contract's inbound callers, does 11,802 transfers a week -> rule: a tracked contract reading zero while its neighbours do not is an address problem first. (11) Infrastructure: exponential backoff and error-aware pagination folded into the main collector (zero 429s, zero paged errors on the largest run yet); the pre-flight validator extended to collector source caught run #23's invalid Binance literal on its first run; 16 busy provider contracts re-paged at 30 pages. 6 of 6 resolvable pre-committed tests resolved as predicted (100%); 13 of 13 prior action items completed. | **(post-publication reconciliation)** Two of the three published failed sources were re-examined: the Binance hot wallet was genuinely truncated (1,000-tx budget covering 4.4 of 7 days) and re-queried in per-day slices, taking hot-to-pipeline from 112,126 to 255,442 EGLD - 48% of everything the desks delivered one-way, and a 2.3x undercount on a figure that resolved a pre-committed test; the 16 'page-capped' provider scans were the shallow first pass and had already been re-paged at 30 pages, so that caveat was false and is withdrawn. Only the known-bad WTAO-3ec9c0 control remains in data_sources_failed.
| 25 | 2026-09-14 | **xEXCHANGE PAUSED ITS MEX/WEGLD POOL AND THE PIPELINE WAS PUT IN MARKET SCALE.** (1) The MEX/WEGLD pair (#2 deepest pool, $442,977) was paused 2026-09-13 16:43 UTC by the router owner. It vanished from /mex/pairs and /mex/economics priced MEX at $0. HMEX supply +672%, 172 failed txs. The hourly chart built for the dashboard showed MEX was already 1.7x before the pause; the first draft's 'pause, then spike' came from daily closes. New rule: read venue state from the contract, not from API absence; exclude HMEX from lending TVL. (2) First exchange-side instrument (CoinGecko ±2% depth + 7d spot volume): delivery is ~2% of CEX turnover, which withdraws run #24's 'invisible bid'. (3) Wave #4 confirmed (UPbit 372,000; net one-way 401,353; desks 56,596; Aug 17 - Sep 14 netted 1,336,721). (4) Staked ratio 46.55%, through the 46.80% branch, while delegation TVL rose and new unDelegations fell by two-thirds. (5) Compound rate 49.44%, first reading below 50%, claims held not sold. (6) Traps caught before publication: provider identity renames (cslabsio -> chainstatelabs, 177,587 EGLD phantom exit), HMEX priced off a frozen pool, a rate-limited 4-provider reward sample. (7) Largest unattributed feeder resolves to Bybit + Gate.io. (8) Dollar-base promotion withdrawn. Tests: 6 resolved (4 as predicted, 1 against, 1 inconclusive), 5 open. |
| 26 | 2026-09-21 | **THE CHAIN HALTED AFTER A VM EXPLOIT, AND EVERY BALANCE METRIC HAD TO BE NETTED.** (1) MultiversX produced no block after 2026-09-19 07:41 UTC. One wallet ran 30 doubling deposit rounds (31 compound calls) against its own contract, ended with 41.3M EGLD (plus 25.6M in the contract, over 2x total supply) and fanned out 9.81M through 12 fresh wallets; 4.86M reached Binance.com, KuCoin and MEXC. MultiversX: attempted exploit of a VM-level atomicity issue, fix on a shadow fork, targeted recovery planned. New rules: check liveness first; check balances against total supply (audit check 9); net incident balances and store net in previous.json. (2) Raw exchange flow +4,913,831, net +53,957. (3) The cash-out route ran through Unknown Whale I, and its OTC-operator label is withdrawn. (4) Hatom's MEX recovery sold 270B MEX of seized collateral through a pause whitelist on the paused pools; EGLD market refilled to 112,295; pools resumed Sep 17, a day late. (5) Pre-halt pipeline quiet: 64,665 one-way, 79% circular; wave #4 netted 467,107. (6) Compound back to 62.68%; withdraw amounts decoded for the first time (108,941 EGLD). (7) Address-poisoning lookalikes of both desks are dusting the routers. Tests: mex-incident-recovery as predicted, compound-regime against, delivery-price-relevance not evaluable. |
