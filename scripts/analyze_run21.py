...
for w,nm in [("otc_hub_trace","WEEK Aug10-17 (7d)"),
...
print(f"  UPbit OTC Desk {b(UP):,.0f} (prev 35,375)")
print(f"  OTC Distribution {b(DI):,.0f} (prev 28,420)")
print(f"  COMBINED {b(UP)+b(DI):,.0f} vs 63,795 -> {b(UP)+b(DI)-63794.75:+,.0f}")
...
print(f"  Mega Whale {b(MEGA):,.4f} (prev 1,099,059.3) delta {b(MEGA)-1099059.2951787356:+,.4f}")
...
print("\n=== REC #6: Unknown Whale I (30d, page cap 40) ===")
wi=D["whale_i_info"]
print(f"  balance {int(wi['balance'])/1e18:,.0f} nonce {wi.get('nonce')} shard {wi.get('shard')} txCount {wi.get('txCount')}")
print(f"  30d inbound txs {len(D['whale_i_inbound_30d'])} outbound {len(D['whale_i_outbound_30d'])}")
def agg(txs,key):
    o={}
    for t in txs:
...
hub=D["otc_hub_trace"]
PIPE=set(hub["inbound"])|set(hub["outbound"])|{UP,DI}
...
def cls(a):
    if a in (UP,DI): return "DESK"
    if a in ROUTERS: return "ROUTER"
    if a in PIPE: return "PIPE"
    if cm.get(a)=="exchange": return "EXCHANGE"
    return "EXTERNAL"
for nm,key,txs in [("INBOUND","sender",D["whale_i_inbound_30d"]),("OUTBOUND","receiver",D["whale_i_outbound_30d"])]:
    ag=agg(txs,key)
    tot_v=sum(v for _,v in ag)
    byc={}
    for a,v in ag: byc[cls(a)]=byc.get(cls(a),0)+v
    print(f"  {nm}: {len(ag)} distinct counterparties, {tot_v:,.0f} EGLD total")
    for c,v in sorted(byc.items(),key=lambda x:-x[1]):
        print(f"     {c:9s} {v:12,.0f}  {100*v/tot_v if tot_v else 0:5.1f}%")
    for a,v in ag[:8]:
        print(f"       {v:11,.0f} {cls(a):9s} {lab(a)[:38]:38s} {a[:14]}")
ts_in=[t.get("timestamp") for t in D["whale_i_inbound_30d"] if t.get("timestamp")]
if ts_in:
...
print("\n=== REC #2: direct-node unwind (Staking SC, 21d) ===")
ssc=D.get("staking_sc_info") or {}
print(f"  Staking SC balance {int(ssc.get('balance','0'))/1e18:,.0f}")
inb=D.get("staking_sc_inbound_21d") or []; outb=D.get("staking_sc_outbound_21d") or []
print(f"  inbound txs {len(inb)} outbound {len(outb)}")
fn_in={}
for t in inb:
    f=t.get("function") or "(transfer)"; fn_in[f]=fn_in.get(f,0)+1
print("  inbound functions:",dict(sorted(fn_in.items(),key=lambda x:-x[1])[:12]))
# unStake / unBond callers with value
for fname in ("unStake","unBondTokens","unStakeTokens","unBond","unStakeNodes","unJail","stake"):
    sub=[t for t in inb if (t.get("function")=="fname") or t.get("function")==fname]
    if sub:
        print(f"  --- {fname}: {len(sub)} calls")
        srcs={}
        for t in sub: srcs[t["sender"]]=srcs.get(t["sender"],0)+1
        for s,c in sorted(srcs.items(),key=lambda x:-x[1])[:8]:
            print(f"      {c:3d}x {lab(s)[:36]:36s} {s[:16]}")
print("  OUTBOUND value txs (unbond payouts) >1000 EGLD:")
for r,v in agg(outb,"receiver")[:10]:
    print(f"     {v:12,.0f} -> {lab(r)[:38]:38s} {r[:16]}")

print("\n=== REC #3: XEGLD redemption destination (XOXNO LSD 7d) ===")
xi=D.get("xoxno_lsd_in_7d") or []; xo=D.get("xoxno_lsd_out_7d") or []
print(f"  inbound txs {len(xi)} outbound {len(xo)}")
fn={}
for t in xi:
    f=t.get("function") or "(transfer)"; fn[f]=fn.get(f,0)+1
print("  inbound functions:",dict(sorted(fn.items(),key=lambda x:-x[1])[:12]))
fno={}
for t in xo:
    f=t.get("function") or "(transfer)"; fno[f]=fno.get(f,0)+1
print("  outbound functions:",dict(sorted(fno.items(),key=lambda x:-x[1])[:12]))
print("  OUTBOUND EGLD by receiver:")
for r,v in agg(xo,"receiver")[:12]:
    print(f"     {v:12,.2f} -> {lab(r)[:38]:38s} {r[:16]}")
print("  INBOUND EGLD by sender:")
for s,v in agg(xi,"sender")[:8]:
    print(f"     {v:12,.2f} <- {lab(s)[:38]:38s} {s[:16]}")
xt=D.get("xegld_transfers_7d")
if isinstance(xt,list):
    print(f"  transfers endpoint returned {len(xt)} rows; functions:",
          dict(sorted({(t.get('function') or '(transfer)'):1 for t in xt}.items())))
...
print("=== status ===")
print(json.dumps(json.load(open("/tmp/run21w/status.json"))["failed"]))
