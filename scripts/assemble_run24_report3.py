...
R["anomalies"]=[
 ...
 {"metric":"binance_hot_to_otc_feeders_egld","current_value":HOT["total_egld"],
  "previous_value":125272.0,"method":"rule_based","severity":"high",
  "change_pct":100*(HOT["total_egld"]-125272.0)/125272.0,
  "description":f"The Binance.com hot wallet sent {f(HOT['total_egld'])} EGLD to known OTC desk feeders and routers within a page-capped {HOT['coverage_days']}-day window - above the 50,000 flow threshold run #23 registered after its balance-based predecessor failed. Binance-labelled feeders delivered {f(FEED.get('Binance',0))} EGLD into the desks. In the same week Binance Staking custody REVERSED its two-week drawdown, +{f(cust['delta'])} to {f(cust['balance'])} on 900,000 out and 1,272,434 back. Funding the pipeline and refilling custody happened simultaneously, which is why only the flow trace answers this question."},
...
R["trend_indicators"]={
 ...
 "regime_shifts":[
  ...
  {"metric":"binance_pipeline_participation","before_value":125272.0,
