...
tests.append(resolve("binance-desk-feed-standing","as_predicted",
  f"Binance.com hot -> known OTC feeders and routers {f(HOT['total_egld'])} EGLD over the full seven days, five times the 50,000 bar (the truncated main pass saw {f(HOT.get('main_pass_egld',0))} and was re-queried in per-day slices); Binance-labelled feeders delivered {f(FEED.get('Binance',0))} into the desks; custody REVERSED +{f(cust['delta'])} to {f(cust['balance'])} on 900,000 out / 1,272,434 back",
...
R["meta_learning"]={
 ...
 "endpoints_that_failed":[
   ...
   "NONE beyond the known-bad control. The two truncation caveats this report first published were both resolved: the Binance hot wallet was re-queried in per-day slices (the figure trebled), and the 16 page-capped provider scans were already re-paged at 30 pages in the same run."],
 "api_quirks":[
 ...
 "data_gaps":[
   f"RESOLVED IN-RUN: the Binance.com hot wallet fills a 1,000-tx budget in 4.4 days. Re-queried in per-calendar-day slices, hot-to-pipeline is {f(HOT['total_egld'])} EGLD against the {f(HOT.get('main_pass_egld',0))} the truncated scan reported - a {HOT['total_egld']/max(HOT.get('main_pass_egld') or 1,1):.1f}x undercount on a figure that resolves a pre-committed test. Daily slicing is now the method for any wallet above ~200 txs/day.",
 ...
 "key_findings":[
   ...
   f"Binance's feed into the desks is a standing programme at {f(HOT['total_egld'])} EGLD hot-to-feeder for the week - {100*HOT['total_egld']/otc['net_one_way']:.0f}% of everything the desks delivered - while its custody balance ROSE {f(cust['delta'])}, breaking the custody-funds-distribution story.",
