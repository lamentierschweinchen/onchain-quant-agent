#!/usr/bin/env python3
"""Run #25 stage 1: compute every derived quantity -> /tmp/run25w/derived.json"""
import json, os, math
...
prev_desk=sum(w["balance_egld"] or 0 for w in prev["watch_addresses"] if w["address"] in DESK_SET)   # run #24 desk balances as stored
upbit_feed=0.0
...
# the extended frame spans four weekly frames: run #22, #23, #24 and this week (#25)
sum_wave_weekly=prev_nets["run22"]+prev_nets["run23"]+prev_nets["run24"]+vn["net_one_way"]
...
O["otc"]={
 ...
 "upbit_feed":upbit_feed,"prev_upbit_feed":462000.0,"feed_by_venue":feed_by_venue,
 ...
 "wave":{"window":"2026-08-17..2026-09-14","gross_out":wave["gross_out"],
...
_p22=json.load(open(f"{REPO}/data/collected/2026-09-07.json"))
...
_prev_lk={p["provider"]:(p.get("locked_egld") or 0) for p in prev["staking_providers"]}
transitions=[]
for p in D["providers"]:
    k=p.get("identity") or p.get("provider")
    if float(p.get("locked",0) or 0)==0 and (_prev_lk.get(k) or 0)>0:
        transitions.append({"provider":k,"address":p.get("provider"),"prev_locked":_prev_lk.get(k),
                            "users":p.get("numUsers"),"nodes":p.get("numNodes")})
...
# (n) PROVIDER IDENTITY RENAME GUARD. cslabsio renamed its identity to
# chainstatelabs; the identity-keyed join booked its full 177,587 EGLD as a
# leaver. The contract address is the stable key - re-join movers on it.
_cur_addr={p.get("provider"):p for p in D["providers"]}
renames=[]
for m in O["staking"]["movers"]:
    if m.get("left_active_set"):
        pv=next((q for q in prev["staking_providers"] if q["provider"]==m["key"]),None)
        cur=_cur_addr.get((pv or {}).get("address"))
        if cur is not None:
            lk=float(cur.get("locked",0) or 0)/1e18
            renames.append({"old_identity":m["key"],"new_identity":cur.get("identity"),
                            "address":cur.get("provider"),"prev_locked":pv["locked_egld"],
                            "locked":lk,"delta":lk-pv["locked_egld"],
                            "users":cur.get("numUsers"),"prev_users":pv.get("num_delegators")})
O["identity_renames"]=renames
_rn={r["old_identity"] for r in renames}
O["staking"]["movers"]=[m for m in O["staking"]["movers"] if m["key"] not in _rn]
for r in renames:
    O["staking"]["moves"].append({"identity":r["new_identity"],"delta":r["delta"],
        "users_delta":(r["users"] or 0)-(r["prev_users"] or 0),"apr":None,"fee":None,
        "locked":r["locked"],"users":r["users"],"nodes":None,"prev_locked":r["prev_locked"],
        "renamed_from":r["old_identity"]})

# (o) THE MEX/WEGLD PAIR WAS PAUSED (2026-09-13 16:43 UTC). /mex/pairs no longer
# lists it, /mex/economics returns price 0, and /tokens/MEX-455c57 carries a
# price 7.7x last week's. Every MEX-dependent figure needs a like-for-like base.
P7=json.load(open(f"{REPO}/data/collected/2026-09-07.json"))
_pmex=next((p for p in P7["mex_pairs"] if p.get("baseSymbol")=="MEX" and p.get("quoteSymbol")=="WEGLD"),None)
_pmex_tvl=(_pmex or {}).get("totalValue") or 0
_pmex_vol=(_pmex or {}).get("volume24h") or 0
_cg_mex=json.load(open(f"{OUT}/cg_mex_chart.json")) if os.path.exists(f"{OUT}/cg_mex_chart.json") else {}
_cgp=[x[1] for x in (_cg_mex.get("prices") or [])]
O["mex_event"]={
  "pair_address":"erd1qqqqqqqqqqqqqpgqa0fsfshnff4n76jhcye6k7uvd7qacsq42jpsp6shh2",
  "paused_at_utc":"2026-09-13T16:43:00Z",
  "paused_by":"erd1ss6u80ruas2phpmr82r42xnkd6rxy40g9jl69frppl4qez9w2jpsqj8x97 (owner of the xExchange router contract that owns the pair)",
  "pause_tx":"b0decfa361def3e21753a1a1f59df1f3a09f7e256a6332e366ba9d4e5f2a8253",
  "last_successful_swap_utc":"2026-09-13T16:39:49Z",
  "failed_txs_7d":172,"remove_liquidity_calls_7d":116,
  "pair_holds_mex":191150546645.27,"pair_holds_wegld":141280.83,
  "prev_pair_tvl_usd":_pmex_tvl,"prev_pair_volume_24h_usd":_pmex_vol,
  "mex_price_tokens_api":(D.get("mex_token") or {}).get("price"),
  "mex_price_mex_economics":D["mex_economics"].get("price"),
  "mex_price_coingecko_now":_cgp[-1] if _cgp else None,
...
if _cgp:
    O["xexchange"]["mex_price"]=_cgp[-1]
    O["xexchange"]["mex_wow"]=100*(_cgp[-1]-pmex)/pmex
...
O["bid"]["pooltvl_ex_mex_wow_pct"]=100*(pooltvl-O["bid"]["prev_pooltvl_ex_mex_usd"])/O["bid"]["prev_pooltvl_ex_mex_usd"]
O["bid"]["pooltvl_egld_ex_mex_wow_pct"]=100*(pooltvl/price-O["bid"]["prev_pooltvl_ex_mex_egld"])/O["bid"]["prev_pooltvl_ex_mex_egld"]
O["bid"]["prev_turnover_ex_mex"]=100*(prev["xexchange"]["volume_24h_usd"]-_pmex_vol)/O["bid"]["prev_pooltvl_ex_mex_usd"]
# Hatom Lending ex-HMEX: HMEX mcap moved $27K -> $1.6M on the MEX price print
_hl_ex=hl-mc("HMEX-df6df7")
_hl_prev_ex=prev["defi_tvl"]["Hatom Lending"]-(P7["tvl_tokens"]["HMEX-df6df7"].get("marketCap") or 0)
...
O["defi"]["inverse_ratio_ex_hmex"]=abs(O["defi"]["hatom_lending_ex_hmex_egld_pct"])/abs(O["macro"]["price_chg"])
O["defi"]["h_tokens"]={t:{"mcap":mc(t),"prev_mcap":P7["tvl_tokens"][t].get("marketCap"),
                          "supply_pct":100*(float(tt[t]["supply"])-float(P7["tvl_tokens"][t]["supply"]))/float(P7["tvl_tokens"][t]["supply"])}
                       for t in HL}
