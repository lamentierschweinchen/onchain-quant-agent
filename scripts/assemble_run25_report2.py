...
R["token_activity"]={
 ...
 "xexchange":{"total_pairs":xx["pairs"],"total_volume_24h_usd":xx["vol"],
   ...
   "mex_pair_event":MEXE},
 "analysis":(
  f"A HATOM MEX MONEY MARKET INCIDENT FROZE THREE MEX POOLS. Hatom's Sep 14 statement ({HATOM_URL}) calls it a MEX money market incident, says user funds are safe and it is contained after a coordinated response with xExchange, and says a recovery plan will unwind the incident-driven activity without losses to users or bad debt. The MEX market and the MEX/EGLD, MEX/USH and MEX/USDC pairs stay paused until about Wednesday; a full incident report is promised and no cause is given yet. "
  f"The chain shows the sequence. The xExchange router owner (erd1ss6u80ruas2p...) paused MEX/WEGLD at 16:43 UTC, three minutes after its last successful swap, then MEX/USH at {HM(PZ['MEX/USH']['ts'])} and MEX/USDC at {HM(PZ['MEX/USDC']['ts'])}. Hatom's MEX market saw its last call at {HM(INC['mex_market_last_call_ts'])}. "
  ...
  f"WHAT HAPPENED IN THE MONEY MARKET, FROM THE CHAIN. One wallet (erd10fe5lv5v...fuqq) spent {f(WA['egld_spent_buying_mex'])} EGLD buying {f(WA['mex_bought']/1e9,1)}B MEX through xExchange's compose-tasks contract between 15:00 and 17:00 UTC, deposited {f(WA['mex_deposited']/1e9,1)}B MEX into Hatom's MEX market ({100*WA['mex_deposited']/MEX_CIRC:.1f}% of circulating MEX) in {len(WA['deposits'])} deposits, and borrowed {f(WA['egld_borrowed'])} EGLD from Hatom's EGLD market in {len(WA['borrows'])} draws from {BORROW_FIRST} to {BORROW_LAST}. The last draw came six minutes before the first pause. A second wallet (erd1fjcgt8g0...8cuk) deposited {f(WB['mex_deposited']/1e9,1)}B MEX and borrowed {f(WB['egld_borrowed'])} EGLD in the same window. "
  f"Buying MEX raises its price, a higher price raises the value of MEX collateral, and more collateral supports more borrowing, so the buy-deposit-borrow sequence is self-reinforcing. Whether that was the mechanism of the incident is for Hatom's report to confirm; the model records the flows and not the intent. Over the same hours a liquidation bot repaid MEX debt {INC['liquidations_on_mex_market']} times on the MEX market as MEX borrowers went underwater, seizing WBTC collateral. MEX supply moved {100*(tk['mex_supply']-tk['mex_prev_supply'])/tk['mex_prev_supply']:+.3f}%, so the week included no mint or burn.\n\n"
...
R["defi_activity"]={
 ...
 "protocol_breakdown":[
  {"protocol":"xExchange","category":"dex","addresses_tracked":16,
   ...
   "notable_events":f"MEX/WEGLD, MEX/USH AND MEX/USDC PAUSED on Sep 13 (16:43, {HM(PZ['MEX/USH']['ts'])}, {HM(PZ['MEX/USDC']['ts'])} UTC) to contain Hatom's MEX money market incident; Hatom expects them to resume by Wednesday. MEX/WEGLD still holds 191B MEX and 141,281 WEGLD, with 172 failed transactions in the week. Volume {f(bid['prev_dexvol_egld'])} -> {f(bid['dexvol_egld'])} EGLD/day, {bid['wegld_usdc_share']:.1f}% WEGLD/USDC. Like-for-like pool TVL ex the paused pair {bid['pooltvl_egld_ex_mex_wow_pct']:+.1f}% in EGLD. WEGLD contract balances {f(df['xexch_egld'])} EGLD ({xe_egld_pct:+.1f}%).",
  ...
  {"protocol":"Hatom Lending","category":"lending","addresses_tracked":13,
   ...
   "notable_events":f"HEADLINE +{df['hatom_lending_egld_pct']:.0f}% IN EGLD IS A MEX PRICE PRINT. HMEX market cap ${f(ht['HMEX-df6df7']['prev_mcap'])} -> ${f(ht['HMEX-df6df7']['mcap'])} (supply {ht['HMEX-df6df7']['supply_pct']:+.0f}%, priced off a MEX quote that no pool sets). Ex-HMEX, EGLD-denominated TVL {hl_ex_pct:+.2f}% on a {pc:+.2f}% price: inverse rule sign correct on a down-week, response ratio {df['inverse_ratio_ex_hmex']:.2f}. The rule's sign came from USD collateral repricing (HUSDC supply {ht['HUSDC-d80042']['supply_pct']:+.1f}%, HWBTC {ht['HWBTC-49ca31']['supply_pct']:+.1f}%) while EGLD depositors redeemed: HEGLD supply {ht['HEGLD-d61095']['supply_pct']:+.2f}%. The EGLD money market contract's -89,886 EGLD is mostly the incident: {f(WA['egld_borrowed']+WB['egld_borrowed'])} EGLD borrowed against MEX collateral on Sep 13, which Hatom's recovery plan says it will unwind. HHTM supply {ht['HHTM-e03ba5']['supply_pct']:+.1f}%.",
 ...
 "sc_deployments":[],
 "analysis":(
  f"THE WEEK'S DEFI STORY IS HATOM'S MEX MONEY MARKET INCIDENT. Hatom says funds are safe and the incident is contained, and the MEX market and three MEX pools are paused until about Wednesday while a recovery plan unwinds it. The MEX/WEGLD pair has been paused since Saturday afternoon, and the aggregators lit up over the 24 hours that include the pause: XOXNO {f(proto.get('XOXNO Aggregator'))} transfers against {f(prev_tr('XOXNO Aggregator'))}, OneDex {f(proto.get('OneDex Swap'))} against {f(prev_tr('OneDex'))}, the JEXchange fees contract {f(proto.get('JEXchange Fees'))} against {f(prev_tr('JEXchange'))}. "
  f"Hatom's MEX market took the deposits: HMEX supply {ht['HMEX-df6df7']['supply_pct']:+.0f}%, almost all of it one wallet's {f(WA['mex_deposited']/1e9,0)}B MEX, which then borrowed {f(WA['egld_borrowed'])} EGLD. That HMEX line is also a trap for the TVL figure. HMEX market cap went from ${f(ht['HMEX-df6df7']['prev_mcap'])} to ${f(ht['HMEX-df6df7']['mcap'])}, priced off a MEX quote no on-chain pool sets, which lifts Hatom Lending's headline EGLD TVL {df['hatom_lending_egld_pct']:+.0f}%. Excluding HMEX the move is {hl_ex_pct:+.2f}%, and that is the figure in the breakdown.\n\n"
  ...
  f"The per-market supplies show where the sign comes from. EGLD depositors redeemed: HEGLD supply {ht['HEGLD-d61095']['supply_pct']:+.2f}%. (The EGLD money market contract also lost 89,886 EGLD, but {f(WA['egld_borrowed']+WB['egld_borrowed'])} of that is incident borrowing, not depositor behaviour.) The markets that grew are dollar and BTC collateral (HUSDC {ht['HUSDC-d80042']['supply_pct']:+.1f}%, HUSDT {ht['HUSDT-6f0914']['supply_pct']:+.1f}%, HWETH {ht['HWETH-b3d17e']['supply_pct']:+.1f}%), and those rise in EGLD terms whenever EGLD falls. "
