#!/usr/bin/env python3
"""Run #11 data collection for MultiversX weekly intel report (2026-06-08)."""
...
# period boundaries (today is 2026-06-08)
SEVEN_DAYS_AGO  = int(datetime(2026,6,1,0,0,0,tzinfo=timezone.utc).timestamp())
ONE_DAY_AGO     = int(datetime(2026,6,7,0,0,0,tzinfo=timezone.utc).timestamp())
FOURTEEN_DAYS_AGO = int(datetime(2026,5,25,0,0,0,tzinfo=timezone.utc).timestamp())
...
def get(path, params=None, retries=2):
    ...
    for attempt in range(retries+1):
        try:
            req = urllib.request.Request(url, headers={"User-Agent":"intel-agent/11"})
...
# accounts to query: exchanges, top non-system non-exchange whales, watch list, OTC pipeline
accounts_to_query = {}
...
# OTC pipeline addresses for ongoing trace + Binance staking custody (carried recommendation)
otc_trace = {
    "erd17l22xekj5lvfulatz20xr0llxky6c8zr923r95qg3pfx668m862skjdveh":"Unknown Whale erd17l22 (OTC source)",
    "erd12tq6ax5k49dkp4lwgxz8ed62gz3xc3xwa30dgxqhke9awz58z2qq07ny36":"OTC source funder erd12tq6ax5k",
    ...
    "erd1rf4hv70arudgzus0ymnnsk7e5j8paw7vc83dwymeryfgn6gl09tql0gpc8":"Binance Staking custody",
...
# erd12tq6ax5k INBOUND (14d) - check if Binance origin still pumping
inbound_funder = get(f"/accounts/erd12tq6ax5k49dkp4lwgxz8ed62gz3xc3xwa30dgxqhke9awz58z2qq07ny36/transactions",
              {"size":50,"after":FOURTEEN_DAYS_AGO,"order":"desc","status":"success",
               "receiver":"erd12tq6ax5k49dkp4lwgxz8ed62gz3xc3xwa30dgxqhke9awz58z2qq07ny36"})
...
# UPbit OTC Desk + OTC Distribution outflows - distribution phase check (carried recommendation)
...
# ZPAY follow-up (ZoidPay - was at 40.8% dominance in run #9 then 8.9% in run #10)
