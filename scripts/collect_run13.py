...
# period boundaries (today is 2026-06-22)
SEVEN_DAYS_AGO  = int(datetime(2026,6,15,0,0,0,tzinfo=timezone.utc).timestamp())
ONE_DAY_AGO     = int(datetime(2026,6,21,0,0,0,tzinfo=timezone.utc).timestamp())
FOURTEEN_DAYS_AGO = int(datetime(2026,6,8,0,0,0,tzinfo=timezone.utc).timestamp())
...
def get(path, params=None, retries=2):
    ...
    for attempt in range(retries+1):
        try:
            req = urllib.request.Request(url, headers={"User-Agent":"intel-agent/13"})
...
# H-token + LSD market caps for DeFi TVL (>=1.0s per run #12 rate-limit rule:
# <1.0s spacing returns HTTP 200 with price=null/marketCap=null silently)
...
json.dump(D, open(f"{REPO}/data/collected/2026-06-22.json","w"))
print("Saved canonical snapshot to data/collected/2026-06-22.json")
