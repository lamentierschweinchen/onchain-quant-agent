#!/usr/bin/env python3
"""Run #15 data collection for MultiversX weekly intel report (2026-06-29)."""
...
# period boundaries (today is 2026-07-06)
SEVEN_DAYS_AGO    = int(datetime(2026,6,29,0,0,0,tzinfo=timezone.utc).timestamp())
ONE_DAY_AGO       = int(datetime(2026,7,5,0,0,0,tzinfo=timezone.utc).timestamp())
FOURTEEN_DAYS_AGO = int(datetime(2026,6,22,0,0,0,tzinfo=timezone.utc).timestamp())
...
def get(path, params=None, retries=2):
    ...
    for attempt in range(retries+1):
        try:
            req = urllib.request.Request(url, headers={"User-Agent":"intel-agent/15"})
...
# erd18mv2z6r2 follow-up - is the mega whale still active? (near 1M threshold)
...
# XEGLD -29% supply-collapse follow-up (run #14 top recommendation):
# trace the XOXNO LSD contract's flows to classify where the freed EGLD went.
# Migration to native delegation/another LSD = bullish; exchange = bearish exit.
...
time.sleep(0.15)
D["xoxno_lsd_outbound"] = get(f"/accounts/{xoxno_lsd_addr}/transactions",
              {"size":50,"after":FOURTEEN_DAYS_AGO,"order":"desc","status":"success",
               "sender":xoxno_lsd_addr})
time.sleep(0.2)
D["xoxno_lsd_transfers"] = get(f"/accounts/{xoxno_lsd_addr}/transfers",
              {"size":50,"after":FOURTEEN_DAYS_AGO,"order":"desc","status":"success"})
time.sleep(0.2)
...
# H-token + LSD market caps for DeFi TVL.
# dataApi-class LSD/stablecoin tokens (SEGLD, SWTAO, USH, XEGLD) silently return
# price=null/marketCap=null under sequential load even at >=1.0s spacing (run #13 quirk).
...
# dataApi null-price re-fetch guard (run #13/#14 rule): isolated re-fetch at >=2.5s,
# up to 4 retries, for any dataApi token that came back null on the sequential pass.
refetch_log = {}
...
# stablecoin supply follow-up (run #13: USDC -0.5%, USDT -1.8%)
for sid in ["USDC-c76f1f","USDT-f8c08c"]:
    D[f"stable_{sid}"] = get(f"/tokens/{sid}")
    time.sleep(0.5)

# EMRS-6e4067 follow-up (run #13 flagged $28M headline mcap, low float - verify traction)
...
json.dump(D, open(f"{REPO}/data/collected/2026-07-06.json","w"))
print("Saved canonical snapshot to data/collected/2026-07-06.json")
