#!/usr/bin/env python3
"""Run #16 data collection for MultiversX weekly intel report (2026-07-13)."""
...
# period boundaries (today is 2026-07-13)
SEVEN_DAYS_AGO    = int(datetime(2026,7,6,0,0,0,tzinfo=timezone.utc).timestamp())
ONE_DAY_AGO       = int(datetime(2026,7,12,0,0,0,tzinfo=timezone.utc).timestamp())
FOURTEEN_DAYS_AGO = int(datetime(2026,6,29,0,0,0,tzinfo=timezone.utc).timestamp())
...
def get(path, params=None, retries=2):
    ...
    for attempt in range(retries+1):
        try:
            req = urllib.request.Request(url, headers={"User-Agent":"intel-agent/16"})
...
json.dump(D, open(f"{REPO}/data/collected/2026-07-13.json","w"))
print("Saved canonical snapshot to data/collected/2026-07-13.json")
