#!/usr/bin/env python3
"""Run #17 data collection for MultiversX weekly intel report (2026-07-20)."""
...
# period boundaries (today is 2026-07-20)
SEVEN_DAYS_AGO    = int(datetime(2026,7,13,0,0,0,tzinfo=timezone.utc).timestamp())
ONE_DAY_AGO       = int(datetime(2026,7,19,0,0,0,tzinfo=timezone.utc).timestamp())
FOURTEEN_DAYS_AGO = int(datetime(2026,7,6,0,0,0,tzinfo=timezone.utc).timestamp())
...
def get(path, params=None, retries=2):
    ...
    for attempt in range(retries+1):
        try:
            req = urllib.request.Request(url, headers={"User-Agent":"intel-agent/17"})
...
json.dump(D, open(f"{REPO}/data/collected/2026-07-20.json","w"))
print("Saved canonical snapshot to data/collected/2026-07-20.json")
