#!/usr/bin/env python3
"""Run #22 data collection for MultiversX intel report (2026-08-24).

CADENCE: previous snapshot is 2026-08-17 -> clean 7-DAY window (2026-08-17 -> 2026-08-24).

Implements run #21's recommendations:
  #2 WHERE DOES THE 229,865 EGLD UNBOND GO? /accounts/{addr}/delegation + paged outbound
  #3 RE-NET THE JULY EPISODE FEED-TO-DRAIN as ONE window (Jul 6 -> Jul 27)
  #4 DO THE 100%-FEE PROVIDERS DEREGISTER? providers + owner wallets + shared counterparty
  #5 DOES THE OTC FEED RESUME? weekly hub trace + wave-window netting (Aug 10 -> Aug 24)
  #6 IS THE USDT DRAIN ONE DESK OR THE BRIDGE? /tokens/USDT-f8c08c/accounts
  #7 WHY IS MEX LEADING EGLD? MEX supply WoW + MEX/WEGLD pool depth in EGLD terms
  #8 Unknown Whale I largest external counterparty erd1qkazru7aw5
  #9/#10 BUILD THE UNBONDING-POOL SERIES: /accounts/{addr}/delegation for wallets
        behind every provider move above ~20K EGLD
"""
...
SEVEN_DAYS_AGO    = ts(2026, 8, 17)   # clean weekly window
WINDOW_END        = ts(2026, 8, 24)
ONE_DAY_AGO       = ts(2026, 8, 23)
THIRTY_DAYS_AGO   = ts(2026, 7, 25)
# run #21 rec #3: re-net the JULY episode feed-to-drain as ONE window
JULY_START, JULY_END = ts(2026, 7, 6), ts(2026, 7, 27)
# run #21 rec #5: wave-window netting - wave #2 feed (Aug 3) through this window's drain
WAVE_START, WAVE_END = ts(2026, 8, 3), ts(2026, 8, 24)
...
D = {"_period": {"report_date": REPORT_DATE,
                 ...
                 "window_days": 7,
                 "note": "clean 7d window 2026-08-17 -> 2026-08-24, comparable to runs #13-#21"}}
ok, failed = [], []
...
print("\n=== HUB TRACE: 7d window (Aug 17 -> Aug 24) ===")
...
# ---------------------------------------------------------------------------
# REC #3 - RE-NET THE JULY EPISODE FEED-TO-DRAIN as ONE window (Jul 6 -> Jul 27)
# ---------------------------------------------------------------------------
print("\n=== HUB TRACE: JULY EPISODE, ONE WINDOW (Jul 6 -> Jul 27) ===")
_ob, _ib = {}, {}
for d_addr, d_label in DESKS.items():
    o = paged_txs(d_addr, JULY_START, before=JULY_END, direction="sender", max_pages=120, tag="julywave")
    i = paged_txs(d_addr, JULY_START, before=JULY_END, direction="receiver", max_pages=120, tag="julywave")
    _ob[d_addr] = {"label":d_label, "txs":o}
    _ib[d_addr] = {"label":d_label, "txs":i}
    print(f"[paged/july] {d_label}: out={len(o)} in={len(i)}")
D["desk_outbound_julywave"] = _ob
D["desk_inbound_julywave"] = _ib
D["otc_hub_trace_julywave"] = hub_trace(_ob, _ib, JULY_START, before=JULY_END)
D["otc_hub_trace_julywave"]["_window"] = "JULY EPISODE Jul 6-27 (runs #16-#18 as ONE window)"
ok.append("July episode re-netted feed-to-drain as one window (rec #3)")

# ---------------------------------------------------------------------------
# REC #5 - WAVE #2 window netting: Aug 3 -> Aug 24 (feed through this drain)
# ---------------------------------------------------------------------------
print("\n=== HUB TRACE: WAVE #2 EXTENDED (Aug 3 -> Aug 24) ===")
_wob, _wib = {}, {}
for d_addr, d_label in DESKS.items():
    o = paged_txs(d_addr, WAVE_START, before=WAVE_END, direction="sender", max_pages=120, tag="wave2ext")
    i = paged_txs(d_addr, WAVE_START, before=WAVE_END, direction="receiver", max_pages=120, tag="wave2ext")
    _wob[d_addr] = {"label":d_label, "txs":o}
    _wib[d_addr] = {"label":d_label, "txs":i}
    print(f"[paged/wave2ext] {d_label}: out={len(o)} in={len(i)}")
D["desk_outbound_wave2ext"] = _wob
D["desk_inbound_wave2ext"] = _wib
D["otc_hub_trace_wave2ext"] = hub_trace(_wob, _wib, WAVE_START, before=WAVE_END)
D["otc_hub_trace_wave2ext"]["_window"] = "WAVE #2 EXTENDED Aug 3-24 (feed-to-drain)"
ok.append("wave #2 extended window netting (rec #5)")

# carry the run #17 peak re-net forward (fixed historical window, measured run #20)
try:
    _prevcol = json.load(open(f"{REPO}/data/collected/2026-08-17.json"))
    for _k in ("otc_hub_trace_peak_run17", "otc_hub_trace_run16", "otc_hub_trace_run18"):
        if _k in _prevcol:
            D[_k] = _prevcol[_k]
    D["_historical_trace_provenance"] = "carried from data/collected/2026-08-17.json (fixed historical windows)"
    ok.append("historical hub-trace windows (carried forward)")
except Exception as e:
    failed.append(f"historical trace carry-forward -> {e}")

# ---------------------------------------------------------------------------
# REC #2 - WHERE DOES THE 229,865 EGLD UNBOND GO?  (highest-value query)
# ---------------------------------------------------------------------------
...
# and the wallet it historically forwarded 12-15K chunks to
_ubw_targets = {}
...
for r, amt in sorted(_ubw_targets.items(), key=lambda x:-x[1])[:5]:
    D[f"unbond_target_{r[:12]}"] = {"address":r, "amount":amt,
                                    "label":label_map.get(r,"Unknown"),
                                    "info":get(f"/accounts/{r}")}
    time.sleep(0.2)
    D[f"unbond_target_{r[:12]}"]["out"] = paged_txs(r, SEVEN_DAYS_AGO, direction="sender", max_pages=6, tag="unbond_hop2")
ok.append("229,865 EGLD unbond destination trace (rec #2 - highest value)")
print(f"[unbond] out7d={len(D['unbond_wallet_out_7d'])} in7d={len(D['unbond_wallet_in_7d'])} targets={len(_ubw_targets)}")

# ---------------------------------------------------------------------------
# REC #9/#10 - UNBONDING POOL SERIES: /accounts/{addr}/delegation for the wallets
#              behind every provider move above ~20K EGLD
# ---------------------------------------------------------------------------
prev_prov = {p["provider"]: p for p in prev.get("staking_providers", [])}
cur_prov = D["providers"] if isinstance(D["providers"], list) else []
big_movers = []
for pr in cur_prov:
    addr = pr.get("provider")
    lk = float(pr.get("locked", 0) or 0)/1e18
    pv = prev_prov.get(addr)
    if not pv:
        continue
    delta = lk - pv.get("locked_egld", 0)
    if abs(delta) >= 20000:
        big_movers.append({"provider":addr, "name":pr.get("identity") or addr,
                           "delta_egld":delta, "locked_egld":lk})
# providers that vanished from the active set entirely
for addr, pv in prev_prov.items():
    if not any(p.get("provider")==addr for p in cur_prov) and pv.get("locked_egld",0) >= 20000:
        big_movers.append({"provider":addr, "name":pv.get("name") or addr,
                           "delta_egld":-pv.get("locked_egld",0), "locked_egld":0, "left_set":True})
D["provider_big_movers"] = big_movers
print(f"[unbonding-pool] {len(big_movers)} providers moved >=20K EGLD")

unbonding_pool = []
seen_wallets = set()
for m in big_movers[:12]:
    txs = paged_txs(m["provider"], SEVEN_DAYS_AGO, direction="receiver", max_pages=8, tag="provmove")
    fn_counts = {}
    for t in txs:
        fn = t.get("function") or "(none)"
        fn_counts[fn] = fn_counts.get(fn, 0) + 1
        if fn in ("unDelegate", "delegate", "withdraw", "reDelegateRewards"):
            snd = t.get("sender")
            if fn == "unDelegate" and snd and snd not in seen_wallets:
                seen_wallets.add(snd)
    m["function_counts"] = fn_counts
    m["undelegate_senders"] = [t.get("sender") for t in txs if t.get("function")=="unDelegate"]
    m["undelegate_txs"] = [{"sender":t.get("sender"),"data":t.get("data"),
                            "timestamp":t.get("timestamp"),"txHash":t.get("txHash")}
                           for t in txs if t.get("function")=="unDelegate"]
for w in list(seen_wallets)[:20]:
    dg = get(f"/accounts/{w}/delegation")
    time.sleep(0.25)
    info = get(f"/accounts/{w}")
    time.sleep(0.2)
    unbonding_pool.append({"wallet":w, "delegation":dg,
                           "balance_egld": int(info.get("balance","0"))/1e18 if isinstance(info,dict) and "balance" in info else None,
                           "label": label_map.get(w, "Unknown")})
D["unbonding_pool"] = unbonding_pool
ok.append("unbonding-pool series: /accounts/{addr}/delegation for provider movers (rec #9/#10)")
print(f"[unbonding-pool] queried {len(unbonding_pool)} wallets behind provider moves")

# ---------------------------------------------------------------------------
# REC #4 - DO THE 100%-FEE PROVIDERS DEREGISTER?
# ---------------------------------------------------------------------------
D["fee100_identities"] = {}
for ident in ("egldstakingprovider", "procryptostaking"):
    D["fee100_identities"][ident] = get("/providers", {"identity":ident})
    time.sleep(0.3)
# owner wallets + shared-counterparty check
owners = []
for ident, plist in D["fee100_identities"].items():
    for pr in (plist if isinstance(plist, list) else []):
        ow = pr.get("owner")
        if ow:
            owners.append({"identity":ident, "provider":pr.get("provider"), "owner":ow})
D["fee100_owners"] = owners
D["fee100_owner_flows"] = {}
for o in owners[:4]:
    ow = o["owner"]
    D["fee100_owner_flows"][ow] = {
        "identity": o["identity"],
        "info": get(f"/accounts/{ow}"),
        "out_30d": paged_txs(ow, THIRTY_DAYS_AGO, direction="sender", max_pages=6, tag="fee100_owner"),
        "in_30d":  paged_txs(ow, THIRTY_DAYS_AGO, direction="receiver", max_pages=6, tag="fee100_owner"),
    }
    time.sleep(0.2)
ok.append("100%-fee provider deregistration + owner counterparty check (rec #4)")

# ---------------------------------------------------------------------------
# REC #6 - IS THE USDT DRAIN ONE DESK OR THE BRIDGE?
# ---------------------------------------------------------------------------
D["usdt_accounts"] = get("/tokens/USDT-f8c08c/accounts", {"size":25})
time.sleep(0.3)
D["usdc_accounts"] = get("/tokens/USDC-c76f1f/accounts", {"size":25})
time.sleep(0.3)
ok.append("stablecoin largest-holder scan (rec #6)")

# ---------------------------------------------------------------------------
# REC #8 - Unknown Whale I's largest external counterparty
# ---------------------------------------------------------------------------
WHALE_I_CP = "erd1qkazru7aw5heycg7c79lwmcnnczgjj2td2dg8j574h586ahwkptq20zlgy"
D["whale_i_info"] = get(f"/accounts/{UNKNOWN_WHALE_I}")
time.sleep(0.2)
D["whale_i_cp_info"] = get(f"/accounts/{WHALE_I_CP}")
time.sleep(0.2)
D["whale_i_cp_out_30d"] = paged_txs(WHALE_I_CP, THIRTY_DAYS_AGO, direction="sender", max_pages=8, tag="whaleI_cp")
D["whale_i_cp_in_30d"]  = paged_txs(WHALE_I_CP, THIRTY_DAYS_AGO, direction="receiver", max_pages=8, tag="whaleI_cp")
ok.append("Unknown Whale I largest external counterparty trace (rec #8)")
...
for w in ["otc_hub_trace","otc_hub_trace_julywave","otc_hub_trace_wave2ext","otc_hub_trace_run16","otc_hub_trace_run18","otc_hub_trace_peak_run17"]:
