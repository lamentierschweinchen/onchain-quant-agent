#!/usr/bin/env python3
"""Run #24 data collection for MultiversX intel report (2026-09-07).

CADENCE: previous snapshot 2026-08-31 -> clean 7-DAY window (2026-08-31 -> 2026-09-07).

Implements run #23's recommendations:
  #3  PRE-FLIGHT RE-QUERY of every source recorded as failed last run, so a
      recovered source is not carried as a standing failure.
  #4  discover_liquid_staking.py is run every week (separate script) and its
      receipt tokens stay in the tracked token list.
  #5  RESOLVE THE DESK-INVENTORY DRAIN - desk balances + wave netting extended
      through this window (Aug 17 -> Sep 7).
  #6  scripts/validate_addresses.py now greps erd1 literals out of scripts/*.py;
      run it BEFORE this collector.
  #7  paged_txs DISTINGUISHES ERROR FROM EMPTY and get() backs off on HTTP 429.
  #9  DEREGISTERED-PROVIDER DELEGATOR SERIES - ledgerbyfigment / stakedinc /
      p2p_org_ delegator counts recorded every week as a decay measurement.
  #10 PROVIDER-SCAN PAGE CAP raised for busy contracts (>250 weekly inbound txs
      get a second, deeper pass) so the unbonding queue stops being a lower bound.
  #11 RE-DERIVE THE JEXCHANGE AGGREGATOR ADDRESS - 0 transfers for three runs
      is a stale address, not a halted protocol.
  #12 DEX turnover in EGLD terms is the primary series (derived in assemble).
  #13 Stablecoin supply tracked for a third consecutive contraction test.
"""
...
SEVEN_DAYS_AGO  = ts(2026, 8, 31)
WINDOW_END      = ts(2026, 9, 7)
ONE_DAY_AGO     = ts(2026, 9, 6)
THIRTY_DAYS_AGO = ts(2026, 8, 8)
...
WAVE_START, WAVE_END = ts(2026, 8, 17), ts(2026, 9, 7)
...
def get(path, params=None, retries=3):
    ...
    for attempt in range(retries+1):
        try:
            req = urllib.request.Request(url, headers={"User-Agent":"intel-agent/24"})
...
D = {"_period": {"report_date": REPORT_DATE,
                 ...
                 "window_days": 7,
                 "note": "clean 7d window 2026-08-31 -> 2026-09-07, comparable to runs #13-#23"}}
ok, failed = [], []
...
print("\n=== HUB TRACE: 7d window (Aug 31 -> Sep 7) ===")
...
print("\n=== HUB TRACE: WAVE #3 EXTENDED (Aug 17 -> Sep 7) ===")
_wob, _wib = {}, {}
for d_addr, d_label in DESKS.items():
...
D["otc_hub_trace_wave3ext"]["_window"] = "WAVE #3 EXTENDED Aug 17 - Sep 7 (feed-to-drain, 3 weeks)"
ok.append("wave #3 extended window netting (Aug 17 - Sep 7)")

try:
    _prevcol = json.load(open(f"{REPO}/data/collected/2026-08-31.json"))
    ...
    if "otc_hub_trace_wave3ext" in _prevcol:
        D["otc_hub_trace_wave3_aug17_31"] = _prevcol["otc_hub_trace_wave3ext"]
    D["_historical_trace_provenance"] = "carried from data/collected/2026-08-31.json (fixed historical windows)"
...
for _a in BINANCE_HOT_ALL:
    ...
    _hot_out += paged_txs(_a, SEVEN_DAYS_AGO, direction="sender", max_pages=20, tag="binance_hot_out")
