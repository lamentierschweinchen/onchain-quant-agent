#!/usr/bin/env python3
"""Run #25 data collection for MultiversX intel report (2026-09-14).

CADENCE: previous snapshot 2026-09-07 -> clean 7-DAY window (2026-09-07 -> 2026-09-14).
...
SEVEN_DAYS_AGO  = ts(2026, 9, 7)
WINDOW_END      = ts(2026, 9, 14)
ONE_DAY_AGO     = ts(2026, 9, 13)
THIRTY_DAYS_AGO = ts(2026, 8, 15)
...
WAVE_START, WAVE_END = ts(2026, 8, 17), ts(2026, 9, 14)
...
def get(path, params=None, retries=3):
    ...
    for attempt in range(retries+1):
        try:
            req = urllib.request.Request(url, headers={"User-Agent":"intel-agent/25"})
...
D = {"_period": {"report_date": REPORT_DATE,
                 ...
                 "window_days": 7,
                 "note": "clean 7d window 2026-09-07 -> 2026-09-14, comparable to runs #13-#24"}}
ok, failed = [], []
...
print("\n=== HUB TRACE: 7d window (Sep 7 -> Sep 14) ===")
...
print("\n=== HUB TRACE: WAVE EXTENDED (Aug 17 -> Sep 14) ===")
_wob, _wib = {}, {}
for d_addr, d_label in DESKS.items():
    o = paged_txs(d_addr, WAVE_START, before=WAVE_END, direction="sender", max_pages=200, tag="wave3ext")
    i = paged_txs(d_addr, WAVE_START, before=WAVE_END, direction="receiver", max_pages=200, tag="wave3ext")
...
ok.append("wave extended window netting (Aug 17 - Sep 14)")

try:
    _prevcol = json.load(open(f"{REPO}/data/collected/2026-09-07.json"))
    ...
    if "otc_hub_trace_wave3ext" in _prevcol:
        D["otc_hub_trace_wave3_aug17_sep7"] = _prevcol["otc_hub_trace_wave3ext"]
    D["_historical_trace_provenance"] = "carried from data/collected/2026-09-07.json (fixed historical windows)"
...
def _feeder_parent(l):
    ...
    return None
_feed_tot = {}
for blk in (D["desk_inbound_wave3ext"],):   # the wave window CONTAINS the 7d window - never sum both
...
RECHECK = {
    ...
    "/tokens/WTAO-3ec9c0 (known-bad identifier)": ("/tokens/WTAO-3ec9c0", None),
}
recheck = {}
