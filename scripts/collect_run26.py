#!/usr/bin/env python3
"""Run #26 data collection for MultiversX intel report (2026-09-21).

CADENCE: previous snapshot 2026-09-14 -> clean 7-DAY window (2026-09-14 -> 2026-09-21).

Run #26 changes (run #25 recommendations):
  #1  MEX incident recovery: pause/resume calls on the three MEX pairs, Hatom MEX
      market callers, incident wallets A/B positions and outbound.
  #5  WITHDRAW AMOUNTS decoded (sampled in followup_run26.py).
  #7  PROVIDERS JOINED ON CONTRACT ADDRESS in the collector; identity renames
      logged as an event list.
  Wave window = wave #4 (UPbit reload Sep 7) -> Sep 21.

Run #25 header retained below for provenance.


Implements run #24's recommendations:
  #1  WAVE #4 OR PROGRAMME COMPLETE - UPbit tranche read off the paginated desk
      inbound leg; wave netting extended through this window (Aug 17 -> Sep 14).
  #2  EXCHANGE-SIDE DEMAND INSTRUMENT - CoinGecko EGLD tickers with depth=true
      (+/-2% order-book cost per venue) for Binance and Bybit, the two venues
      the pipeline delivers into. Crude, third-party, and the first measurement
      of the order book this model has ever taken.
  #3  BINANCE HOT WALLET QUERIED BY CALENDAR DAY in the main pass (the run #24
      followup method), deduped by txHash, so hot-to-pipeline is not a lower bound.
  #5  ATTRIBUTE THE UNATTRIBUTED FEEDERS - two hops back from the largest desk
      feeders whose parent venue is unresolved.
  #6  JEXCHANGE ROUTER promoted into the protocol set (re-derived run #24).
  #8  discover_liquid_staking.py run weekly (separate script); holders recorded.
  Carried: pre-flight recheck, paged_txs error/empty distinction, 429 backoff,
  all-provider unDelegate scan with deep re-page, dereg delegator series.
"""
...
SEVEN_DAYS_AGO  = ts(2026, 9, 14)
WINDOW_END      = ts(2026, 9, 21)
ONE_DAY_AGO     = ts(2026, 9, 20)
THIRTY_DAYS_AGO = ts(2026, 8, 22)
# wave #3: feed restarted Aug 17; extend feed-to-drain through this window so the
# desk-inventory drain test (run #23 rec #5) is measured on the wave, not the week.
WAVE_START, WAVE_END = ts(2026, 9, 7), ts(2026, 9, 21)   # wave #4: UPbit 372K reload landed in the Sep 7-14 window
...
def get(path, params=None, retries=3):
    """GET with exponential backoff on HTTP 429 (run #23 rec #7).

    The run #23 followup pass used 1s doubling to 12s and hit zero errors while
    the main pass, at a flat 0.22s, tripped the rate limiter during the
    all-provider scan. Backoff is now the default, not the repair path.
    """
    url = API + path
    if params:
    ...
    for attempt in range(retries+1):
        try:
            req = urllib.request.Request(url, headers={"User-Agent":"intel-agent/26"})
...
def paged_txs(addr, after, before=None, direction="sender", page=50, max_pages=60, tag=""):
    """Paginate an account's transactions.

    run #23 rec #7: an HTTP error is NOT an empty window. The old helper broke
    out of the loop identically for both, so a 400 or a 429 read as "nothing
    happened" and nulled three findings in one run. Errors are now recorded in
    PAGED_ERRORS and the partial result is tagged.
    """
    out, frm = [], 0
    hit_cap = True
    errored = None
    for _ in range(max_pages):
        ...
        if isinstance(batch, dict) and "__error__" in batch:
            errored = batch["__error__"]
            hit_cap = False
            PAGED_ERRORS.append({"address":addr, "direction":direction, "tag":tag,
                                 "page_from":frm, "error":errored,
                                 "partial_txs":len(out)})
            print(f"  [PAGE-ERROR] {addr[:14]} {direction} {tag}: {errored} after {len(out)} txs")
            break
...
D = {"_period": {"report_date": REPORT_DATE,
                 ...
                 "window_days": 7,
                 "note": "clean 7d window 2026-09-14 -> 2026-09-21, comparable to runs #13-#25"}}
ok, failed = [], []
...
# run #23 shipped an INVALID checksum here and got HTTP 400 all week. These
# three are the checksummed Binance.com hot wallets from known-addresses.json,
# verified by the run #23 followup pass and now covered by the pre-flight.
BINANCE_HOT_ALL = [
...
print("\n=== HUB TRACE: 7d window (Sep 14 -> Sep 21) ===")
...
print("\n=== HUB TRACE: WAVE #4 (Sep 7 -> Sep 21) ===")
_wob, _wib = {}, {}
for d_addr, d_label in DESKS.items():
    o = paged_txs(d_addr, WAVE_START, before=WAVE_END, direction="sender", max_pages=200, tag="wave4")
    i = paged_txs(d_addr, WAVE_START, before=WAVE_END, direction="receiver", max_pages=200, tag="wave4")
    _wob[d_addr] = {"label":d_label, "txs":o}
    _wib[d_addr] = {"label":d_label, "txs":i}
    print(f"[paged/wave4] {d_label}: out={len(o)} in={len(i)}")
D["desk_outbound_wave4"] = _wob
D["desk_inbound_wave4"] = _wib
D["otc_hub_trace_wave4"] = hub_trace(_wob, _wib, WAVE_START, before=WAVE_END)
D["otc_hub_trace_wave4"]["_window"] = "WAVE #4 Sep 7 - Sep 21 (feed-to-drain, 2 weeks)"
ok.append("wave #4 window netting (Sep 7 - Sep 21)")

try:
    _prevcol = json.load(open(f"{REPO}/data/collected/2026-09-14.json"))
    for _k in ("otc_hub_trace_peak_run17","otc_hub_trace_run16","otc_hub_trace_run18",
               "otc_hub_trace_julywave","otc_hub_trace_wave2ext"):
        if _k in _prevcol:
            D[_k] = _prevcol[_k]
    # run #23's wave-3 window (Aug 17-31) is kept alongside this run's extension
    # (Aug 17 - Sep 7) so the drain can be read as a delta on the same frame.
    if "otc_hub_trace_wave3_aug17_31" in _prevcol:
        D["otc_hub_trace_wave3_aug17_31"] = _prevcol["otc_hub_trace_wave3_aug17_31"]
    # run #24's wave-3 frame (Aug 17 - Sep 7) kept so this run's extension to
    # Sep 14 can be read as a delta on the identical frame.
    if "otc_hub_trace_wave3_aug17_sep7" in _prevcol:
        D["otc_hub_trace_wave3_aug17_sep7"] = _prevcol["otc_hub_trace_wave3_aug17_sep7"]
    # run #25's four-week frame (waves #3+#4, Aug 17 - Sep 14), fixed history now
    if "otc_hub_trace_wave3ext" in _prevcol:
        D["otc_hub_trace_wave34_aug17_sep14"] = {k: v for k, v in _prevcol["otc_hub_trace_wave3ext"].items()
                                                 if k in ("venue_netting", "_window")}
    D["_historical_trace_provenance"] = "carried from data/collected/2026-09-14.json (fixed historical windows)"
...
# (cslabsio -> chainstatelabs booked 177,587 EGLD as a leaver in run #25).
prev_prov = {(p.get("address") or p["provider"]): p for p in prev.get("staking_providers", [])}
cur_all = D["providers"] if isinstance(D["providers"], list) else []
def pkey(p): return p.get("provider")
cur_keys = {pkey(p) for p in cur_all}
D["provider_identity_changes"] = []
for p in cur_all:
    pv = prev_prov.get(pkey(p))
    if pv is None: continue
    old_id = pv.get("provider") if pv.get("provider") != pv.get("address") else None
    new_id = p.get("identity")
    if (old_id or None) != (new_id or None):
        D["provider_identity_changes"].append({"address": pkey(p), "from": old_id, "to": new_id,
                                               "locked_egld": float(p.get("locked",0) or 0)/1e18})
print(f"[join] identity changes: {D['provider_identity_changes']}")
...
for p in cur_all:
    ...
    if abs(d) >= 5000:
        movers.append({"key":k,"identity":p.get("identity"),"address":p.get("provider"),"delta_egld":d,"locked_egld":lk,
...
D["deregistered_providers"] = [
    {"key":pkey(p),"address":p.get("provider"),"nodes":p.get("numNodes"),
     ...
     "identity":p.get("identity"),"prev_locked":prev_prov.get(pkey(p),{}).get("locked_egld"),
     "prev_users":prev_prov.get(pkey(p),{}).get("num_delegators")}
    for p in cur_all
    # run #23 quirk: a FULLY exited operator (p2p_org_) reads numNodes 0 after
    # unBondNodes, so the run #22 signature missed it. Widened to numUsers too.
    if float(p.get("locked",0) or 0) == 0 and (int(p.get("numNodes",0) or 0) > 0
                                               or int(p.get("numUsers",0) or 0) > 0)]
...
for i, p in enumerate(scan_set):
    ...
    # rec #10: a contract that fills the 6-page budget is exactly the one where
    # most unbonding happens, so the queue figure was a lower bound on the busy
    # providers only. Re-scan those at a much deeper budget.
    if len(txs) >= 6*50:
        deep = paged_txs(addr, SEVEN_DAYS_AGO, before=WINDOW_END, direction="receiver",
                         max_pages=30, tag="provscan_deep")
        if len(deep) > len(txs):
            print(f"  [DEEP] {str(k)[:28]}: {len(txs)} -> {len(deep)} txs")
            txs = deep
            DEEP_SCANNED.append({"provider":k, "address":addr, "txs":len(deep)})
    fn = {}; legs = []
    for t in txs:
        ...
        if f in ("unDelegate","withdraw","unStakeNodes"):
            ...
            try:
                ...
                pass
            if f == "withdraw":
                WITHDRAW_LEGS.append({"provider":k,"identity":p.get("identity"),"sender":snd,
                                      "timestamp":t.get("timestamp"),"txHash":t.get("txHash")})
            if f == "unDelegate":
...
D["withdraw_legs"] = WITHDRAW_LEGS
D["provider_scan_count"] = len(scan_set)
D["provider_deep_scanned"] = DEEP_SCANNED
...
_hot_out, _hot_in, _hot_bal = [], [], {}
_hot_seen = set()
_hot_daily = []
for _a in BINANCE_HOT_ALL:
    _inf = get(f"/accounts/{_a}"); time.sleep(0.2)
    _hot_bal[_a] = int(_inf.get("balance","0"))/1e18 if isinstance(_inf, dict) and "balance" in _inf else None
    # run #24 rec #3: the busiest hot wallet fills a 1,000-tx budget in ~4.4 days,
    # so the outbound leg is paged ONE CALENDAR DAY AT A TIME and deduped by hash.
    for _d in range(7):
        _aft = SEVEN_DAYS_AGO + 86400*_d
        _bef = _aft + 86400
        _day = paged_txs(_a, _aft, before=_bef, direction="sender", max_pages=40, tag="binance_hot_out_day")
        _new = [t for t in _day if t.get("txHash") not in _hot_seen]
        for t in _new: _hot_seen.add(t.get("txHash"))
        _hot_out += _new
        _hot_daily.append({"address":_a, "day_start_ts":_aft, "txs":len(_day)})
    _hot_in  += paged_txs(_a, SEVEN_DAYS_AGO, direction="receiver", max_pages=20, tag="binance_hot_in")
D["binance_hot_daily_slices"] = _hot_daily
D["binance_hot_out"] = _hot_out
D["binance_hot_in"]  = _hot_in
D["binance_hot_balances"] = _hot_bal
_big_hot = {}
...
for a, rec in D["otc_hub_trace_wave4"]["outbound"].items():
    if rec["amount"] >= 2000 and not venue_of(a):
...
proto_contracts = {
    ...
    # run #24 rec #6: the re-derived live router and the four contracts behind it
    "JEXchange Router": "erd1qqqqqqqqqqqqqpgq360nakqgsp5zkmguptucpjy6n4n3du7e5snsd2swzq",
    "JEXchange Router 2": "erd1qqqqqqqqqqqqqpgqahn5kpu95tsd8sr9swtp7kj30c5nc0k36avst0muvv",
    "JEXchange Router 3": "erd1qqqqqqqqqqqqqpgqahfmv4apudlzgawgcvp2zr65dtufqfmy6avsazkewu",
    "JEXchange Router 4": "erd1qqqqqqqqqqqqqpgqypzse2xtacw5a2wnjv2v2n7sumyymdm96avsv022ct",
    "JEXchange Router 5": "erd1qqqqqqqqqqqqqpgqc75pqw9ye84c7v3cncnhwp7ys5kcet7m6avscyfyj3",
...
# JEX pre-committed test is on a 7-day count - measure it on the same basis
D["jex_router_7d"] = {}
for name in [n for n in proto_contracts if n.startswith("JEXchange Router")]:
    D["jex_router_7d"][name] = get(f"/accounts/{proto_contracts[name]}/transfers/count",
                                   {"after": SEVEN_DAYS_AGO, "before": WINDOW_END})
    time.sleep(0.2)
ok.append("JEXchange router 7d transfer counts (run #24 rec #6)")

# ---------------------------------------------------------------------------
# REC #5 - ATTRIBUTE THE UNATTRIBUTED FEEDERS: two hops BACK from the largest
# desk feeders whose parent venue is not resolved (7d window and the wave).
# ---------------------------------------------------------------------------
def _feeder_parent(l):
    if not l: return None
    for v in ["UPbit","Binance","Bybit","Gate","KuCoin","Bitget","Coinbase","Crypto.com","MEXC"]:
        if v in l: return v
    return None
_feed_tot = {}
for blk in (D["desk_inbound_wave4"],):   # the wave window CONTAINS the 7d window - never sum both
    for d_addr, rec in blk.items():
        for t in rec["txs"]:
            s_ = t.get("sender"); v_ = int(t.get("value","0"))/1e18
            if v_ <= 0 or s_ in DESKS: continue
            if _feeder_parent(label_map.get(s_)): continue
            _feed_tot[s_] = _feed_tot.get(s_, 0) + v_
back = {}
for fa, famt in sorted(_feed_tot.items(), key=lambda x:-x[1])[:8]:
    hop1 = paged_txs(fa, WAVE_START, before=WINDOW_END, direction="receiver", max_pages=10, tag="feeder_back1")
    src = {}
    for t in hop1:
        v_ = int(t.get("value","0"))/1e18
        if v_ > 0 and t.get("sender") not in DESKS:
            src[t["sender"]] = src.get(t["sender"], 0) + v_
    hop2 = {}
    for sa, samt in sorted(src.items(), key=lambda x:-x[1])[:4]:
        if _feeder_parent(label_map.get(sa)) or cat_map.get(sa) == "exchange":
            hop2[sa] = {"amount": samt, "label": label_map.get(sa), "resolved_direct": True}
            continue
        h2 = paged_txs(sa, WAVE_START, before=WINDOW_END, direction="receiver", max_pages=6, tag="feeder_back2")
        s2 = {}
        for t in h2:
            v_ = int(t.get("value","0"))/1e18
            if v_ > 0: s2[t["sender"]] = s2.get(t["sender"], 0) + v_
        inf = get(f"/accounts/{sa}"); time.sleep(0.18)
        hop2[sa] = {"amount": samt, "label": label_map.get(sa), "resolved_direct": False,
                    "nonce": inf.get("nonce") if isinstance(inf, dict) else None,
                    "balance": int(inf.get("balance","0"))/1e18 if isinstance(inf, dict) and "balance" in inf else None,
                    "sources": {k: {"amount": v, "label": label_map.get(k)}
                                for k, v in sorted(s2.items(), key=lambda x:-x[1])[:5]}}
    inf = get(f"/accounts/{fa}"); time.sleep(0.18)
    back[fa] = {"into_desks_egld": famt, "label": label_map.get(fa),
                "nonce": inf.get("nonce") if isinstance(inf, dict) else None,
                "balance": int(inf.get("balance","0"))/1e18 if isinstance(inf, dict) and "balance" in inf else None,
                "hop1_sources": {k: {"amount": v, "label": label_map.get(k)}
                                 for k, v in sorted(src.items(), key=lambda x:-x[1])[:6]},
                "hop2": hop2}
    print(f"[feeder-back] {fa[:14]} {famt:,.0f} <- {[ (label_map.get(k) or k[:12], round(v)) for k,v in sorted(src.items(), key=lambda x:-x[1])[:3]]}")
D["feeder_backtrace"] = back
ok.append("unattributed feeder two-hop back-trace (run #24 rec #5)")

# ---------------------------------------------------------------------------
# REC #2 - EXCHANGE-SIDE DEMAND INSTRUMENT (third-party order-book depth)
# ---------------------------------------------------------------------------
D["cg_tickers"] = getraw("https://api.coingecko.com/api/v3/coins/elrond-erd-2/tickers?depth=true&order=volume_desc")
time.sleep(1.5)
D["cg_market_chart"] = getraw("https://api.coingecko.com/api/v3/coins/elrond-erd-2/market_chart?vs_currency=usd&days=14&interval=daily")
time.sleep(1.5)
if isinstance(D["cg_tickers"], dict) and "__error__" not in D["cg_tickers"]:
    ok.append("coingecko tickers depth=true (exchange-side depth, run #24 rec #2)")
else:
    failed.append(f"coingecko tickers -> {(D['cg_tickers'] or {}).get('__error__')}")
if isinstance(D["cg_market_chart"], dict) and "__error__" not in D["cg_market_chart"]:
    ok.append("coingecko market_chart 14d (aggregate spot volume)")
else:
    failed.append(f"coingecko market_chart -> {(D['cg_market_chart'] or {}).get('__error__')}")
...
# ---------------------------------------------------------------------------
# REC #9 - DEREGISTERED-PROVIDER DELEGATOR SERIES (participation-inertia decay)
# Three providers at zero locked stake with delegators still attached, at three
# different durations. Four numbers a week turns a qualitative finding into a
# measured decay rate.
# ---------------------------------------------------------------------------
DEREG_WATCH = {p_["identity"]: p_ for p_ in prev.get("deregistered_providers", [])}
_cur_by_ident = {p_.get("identity"): p_ for p_ in cur_all if p_.get("identity")}
_cur_by_addr  = {p_.get("provider"): p_ for p_ in cur_all if p_.get("provider")}
dereg_series = []
for ident, pv in DEREG_WATCH.items():
    cur = _cur_by_ident.get(ident) or _cur_by_addr.get(pv.get("address"))
    if cur is None:
        cur = get("/providers", {"identity": ident})
        time.sleep(0.3)
        cur = cur[0] if isinstance(cur, list) and cur else None
    dereg_series.append({
        "identity": ident,
        "address": pv.get("address"),
        "prev_users": pv.get("num_users"),
        "users": (cur or {}).get("numUsers"),
        "users_delta": ((cur or {}).get("numUsers") or 0) - (pv.get("num_users") or 0)
                        if cur and cur.get("numUsers") is not None else None,
        "num_nodes": (cur or {}).get("numNodes"),
        "locked_egld": float((cur or {}).get("locked", 0) or 0)/1e18 if cur else None,
        "users_at_deregistration": pv.get("users_at_deregistration"),
        "zero_locked_since": pv.get("zero_locked_since"),
        "still_in_providers_list": cur is not None,
    })
    print(f"[dereg-series] {ident}: users {pv.get('num_users')} -> {(cur or {}).get('numUsers')}")
D["deregistered_provider_series"] = dereg_series
ok.append("deregistered-provider delegator series (rec #9)")

# ---------------------------------------------------------------------------
# REC #11 - RE-DERIVE THE JEXCHANGE AGGREGATOR ADDRESS
# The hardcoded aggregator has returned 0 transfers for three consecutive runs
# while the fees contract reports thousands. That pattern is a stale address.
# ---------------------------------------------------------------------------
JEX_FEES = "erd1272et87h3sa7hlg5keuswh50guz2ngmd6lhmjxkwwu0ah6gdds5qhka964"
JEX_OLD_AGG = "erd1qqqqqqqqqqqqqpgqqvs2jvf64wzcz2836er0j98l3ytshpcr5sns997aga"
jex = {"old_aggregator": JEX_OLD_AGG,
       "old_aggregator_account": get(f"/accounts/{JEX_OLD_AGG}")}
time.sleep(0.2)
jex["old_aggregator_transfers_7d"] = get(f"/accounts/{JEX_OLD_AGG}/transfers/count",
                                         {"after": SEVEN_DAYS_AGO})
time.sleep(0.2)
# Who pays the fees contract? The senders on its inbound leg are the live
# aggregator(s) - this is the run #21 "trace through the callers" rule.
_jex_in = get(f"/accounts/{JEX_FEES}/transfers",
              {"size":50, "after":SEVEN_DAYS_AGO, "receiver":JEX_FEES, "order":"desc"})
time.sleep(0.25)
_senders = {}
if isinstance(_jex_in, list):
    for t in _jex_in:
        srcs = [t.get("sender")]
        # a fee payment usually arrives as an SC result of an aggregator call
        if t.get("originalTxHash"):
            srcs.append(t.get("originalSender") or None)
        for sdr in srcs:
            if sdr and sdr != JEX_FEES:
                _senders[sdr] = _senders.get(sdr, 0) + 1
jex["fees_inbound_senders"] = dict(sorted(_senders.items(), key=lambda x:-x[1])[:15])
jex["candidates"] = {}
for cand, n in list(sorted(_senders.items(), key=lambda x:-x[1]))[:5]:
    if not cand.startswith("erd1qqqq"):
        continue
    jex["candidates"][cand] = {
        "hits": n,
        "account": get(f"/accounts/{cand}"),
    }
    time.sleep(0.2)
    jex["candidates"][cand]["transfers_7d"] = get(f"/accounts/{cand}/transfers/count",
                                                  {"after": SEVEN_DAYS_AGO})
    time.sleep(0.2)
D["jexchange_rederivation"] = jex
ok.append("JEXchange aggregator re-derivation (rec #11)")
print(f"[jex] old agg transfers7d={jex['old_aggregator_transfers_7d']}, "
      f"{len(jex['candidates'])} contract candidates from the fees inbound leg")

# ---------------------------------------------------------------------------
# REC #3 - PRE-FLIGHT: RE-QUERY LAST RUN'S FAILURES
# Three of run #23's four recorded failures were transient and had already
# recovered by the followup pass, but were still published as failures.
# ---------------------------------------------------------------------------
RECHECK = {
    "/tokens/WTAO-4f5363": ("/tokens/WTAO-4f5363", None),
    "/accounts/{BINANCE_HOT}": (f"/accounts/{BINANCE_HOT}", None),
    "/accounts/{UNBOND_WALLET}/delegation": (f"/accounts/{UNBOND_WALLET}/delegation", None),
    "/mex/pairs/MEX-455c57/WEGLD-bd4d79 (404 while paused, run #25)": ("/mex/pairs/MEX-455c57/WEGLD-bd4d79", None),
}
recheck = {}
for name, (path, params) in RECHECK.items():
    r = get(path, params)
    time.sleep(0.25)
    okk = not (isinstance(r, dict) and "__error__" in r)
    recheck[name] = {"ok": okk, "error": None if okk else r.get("__error__")}
    print(f"[recheck] {name}: {'RECOVERED/OK' if okk else 'still failing'}")
D["prior_failure_recheck"] = recheck
ok.append("pre-flight re-query of last run's failed sources (rec #3)")


# ---------------------------------------------------------------------------
# RUN #25 REC #1 - MEX INCIDENT RECOVERY
# ---------------------------------------------------------------------------
ROUTER_OWNER = "erd1ss6u80ruas2phpmr82r42xnkd6rxy40g9jl69frppl4qez9w2jpsqj8x97"
...
MEX_PAIRS = {"MEX/WEGLD":"erd1qqqqqqqqqqqqqpgqa0fsfshnff4n76jhcye6k7uvd7qacsq42jpsp6shh2",
             "MEX/USH":"erd1qqqqqqqqqqqqqpgqyghvzwv9qyzq95vwhvk4tafg945r8hcf2jps0x7me7",
             "MEX/USDC":"erd1qqqqqqqqqqqqqpgqv0c22xku4vd5ugfh0u53tqufmdkqx5nv2jps8vsccx"}
INC_WALLETS = {"A":"erd10fe5lv5vwslmua46zgx9vfy79575al3cy2z6t26j94flqymccdpqz0fuqq",
               "B":"erd1fjcgt8g02ejvhqc5hye8k0zqws2j5g4mw3vxvyy38tv9llpkwt3q428cuk"}
inc = {"router_owner_calls": paged_txs(ROUTER_OWNER, ts(2026,9,13), before=WINDOW_END, direction="sender", max_pages=4, tag="mex_owner")}
inc["pairs"] = {}
for nm, pa in MEX_PAIRS.items():
    rec = {"address": pa, "account": get(f"/accounts/{pa}")}; time.sleep(0.2)
    rec["tokens"] = get(f"/accounts/{pa}/tokens", {"size": 10}); time.sleep(0.2)
    rec["txs_7d_count"] = get(f"/accounts/{pa}/transactions/count", {"after": SEVEN_DAYS_AGO}); time.sleep(0.2)
    rec["failed_7d_count"] = get(f"/accounts/{pa}/transactions/count", {"after": SEVEN_DAYS_AGO, "status": "fail"}); time.sleep(0.2)
    rec["recent"] = get(f"/accounts/{pa}/transactions", {"size": 25, "after": SEVEN_DAYS_AGO, "order": "desc"}); time.sleep(0.2)
    inc["pairs"][nm] = rec
inc["mex_mm_txs"] = paged_txs(MEX_MM, SEVEN_DAYS_AGO, before=WINDOW_END, direction="receiver", max_pages=10, tag="mex_mm")
inc["mex_mm_account"] = get(f"/accounts/{MEX_MM}"); time.sleep(0.2)
inc["wallets"] = {}
for k, w in INC_WALLETS.items():
    inc["wallets"][k] = {"address": w, "account": get(f"/accounts/{w}"),
                         "tokens": get(f"/accounts/{w}/tokens", {"size": 25}),
                         "out": paged_txs(w, SEVEN_DAYS_AGO, before=WINDOW_END, direction="sender", max_pages=6, tag="inc_out"),
                         "in": paged_txs(w, SEVEN_DAYS_AGO, before=WINDOW_END, direction="receiver", max_pages=6, tag="inc_in")}
    time.sleep(0.3)
inc["hmex_token"] = get("/tokens/HMEX-df6df7"); time.sleep(0.3)
inc["cg_mex_chart"] = getraw("https://api.coingecko.com/api/v3/coins/maiar-dex/market_chart?vs_currency=usd&days=14&interval=daily")
time.sleep(1.5)
D["mex_incident_followup"] = inc
ok.append("MEX incident recovery follow-up (run #25 rec #1)")
print(f"[mex] owner calls {len(inc['router_owner_calls'])}, mex_mm txs {len(inc['mex_mm_txs'])}")

# --- save ------------------------------------------------------------------
D["_pagecap_terminations"] = PAGECAP_LOG
D["_paged_errors"] = PAGED_ERRORS
D["_api_errors"] = API_ERRORS
...
for w in ["otc_hub_trace","otc_hub_trace_wave4"]:
...
print("=== PAGED ERRORS:", json.dumps(PAGED_ERRORS, indent=1)[:2000])
