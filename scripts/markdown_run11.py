#!/usr/bin/env python3
import json
R=json.load(open("/Users/ls/Documents/MultiversX/projects/onchain-quant-agent/reports/2026-06-08.json"))
...
w(f"**Period**: 2026-06-01 → 2026-06-08 (7 days)")
...
w("|---|---|---|")
w(f"| Price regime | 🔴 Floor BROKE | {d['price_change_pct']:+.1f}% WoW to ${e['egld_price_usd']:.2f}; z=-3.32σ (HIGH); -37% from May 4 peak; decoupled from BTC/ETH |")
w(f"| Exchange flows | 🔴 Net INFLOW | Net {wi['exchange_flows']['net_change_egld']:+,.0f} EGLD; off-exchange accumulation thesis collapsed; sell-into-decline pattern |")
w(f"| Binance staking custody | 🟡 STALLED | 3.51M EGLD UNCHANGED; 3-week accumulation streak ended; 779K cum parked, awaiting decisive move |")
w(f"| OTC pipeline | 🔴 Distributing | UPbit OTC -14K, OTC Dist -12.4K (combined -26.4K); run #10 prediction validated wk 1 |")
w(f"| DEX activity | 🟢 +12% volume | ${ta['xexchange']['total_volume_24h_usd']/1e3:.0f}K (vs $108K last wk); WEGLD/USDC dominance 93% (normalized) |")
...
w(f"| Yield-chase migration | ⚫ REGIME ENDED | Cohort net -2.6K; 3 of 5 prior leaders reversed; only ninjastaking sustained (+38K cum) |")
w(f"| Delegator base | 🔴 -447 WoW | Largest absolute drop in tracking history; capitulation pattern emerging |")
w(f"| Bilateral inverse | 🟡 WEAKENING | Hatom Lending +3.3% EGLD during -15.7% drop (vs +8.3%/-11.8% last wk) |")
w(f"| Token holder growth | 🔴 -11 weeks | Established airdrop-decay baseline; -21 to -139 across top-10 |")
w(); w("---"); w()
...
w(f"| BTC | ${m['btc_price_usd']:,} | $63,135 | +1.33% (24h) |")
w(f"| ETH | ${m['eth_price_usd']:,.0f} | $1,668 | +3.56% (24h) |")
w()
w("### Activity"); w()
...
ml=R["meta_learning"]
w(f"- **Run #{ml['run_number']}** · {ml['action_items_completed']}/{ml['action_items_from_previous']} prior action items completed · {ml['new_addresses_discovered']} new addresses discovered")
...
open("/Users/ls/Documents/MultiversX/projects/onchain-quant-agent/reports/2026-06-08.md","w").write("\n".join(L)+"\n")
print("WROTE reports/2026-06-08.md  lines:",len(L))
