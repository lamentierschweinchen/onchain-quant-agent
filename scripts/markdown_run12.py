#!/usr/bin/env python3
import json
R=json.load(open("/Users/ls/Documents/MultiversX/projects/onchain-quant-agent/reports/2026-06-15.json"))
...
w(f"**Period**: 2026-06-08 -> 2026-06-15 (7 days)")
...
w("|---|---|---|")
w(f"| Price action | Bounce | {d['price_change_pct']:+.2f}% WoW to ${e['egld_price_usd']:.2f}; FIRST up-week after 5 down-weeks; BTC +2.0%, ETH +2.6% (back IN LINE with crypto-macro) |")
w(f"| Exchange flows | INFLOW (2nd wk) | Net {wi['exchange_flows']['net_change_egld']:+,.0f} EGLD; 2-week confirmation of off-exchange-thesis reversal; sell-into-bounce pattern |")
w(f"| Binance staking custody | STALLED (2nd wk) | 3.51M EGLD UNCHANGED; 779K parked since run #7 ($2.33M at current price) |")
w(f"| Delegator base | CRATERED | -4,003 WoW (-2.24%) - largest single-week drop in tracking by 9x |")
w(f"| Yield-chase cohort | FULLY UNWOUND | Net -17K; ninjastaking (sole 5-wk sustained gainer) reversed -10.5K |")
w(f"| DEX activity | COLLAPSED | {usd(ta['xexchange']['total_volume_24h_usd'])} (-55% WoW); largest DEX volume drop in tracking |")
w(f"| Engagement composite | BEARISH | Price up while delegator/DEX-volume/yield-chase all collapse = 'exit liquidity bounce' signature |")
...
w(f"| Token holder growth | -12 weeks | Established airdrop-decay baseline; -21 to -139 across top-10 |")
w(f"| Reward compounding | DECLINING | Compound rate 59.14% (vs 61.90% last run); institutional claims 100% SOLD (small sample of 2) |")
w(f"| Bilateral inverse | N/A (small move) | Price +1.4% below 5% meaningful-move threshold; rule not testable this run |")
w(); w("---"); w()
...
w(f"| BTC | ${m['btc_price_usd']:,} | $64,406 | +2.00% (24h) |")
w(f"| ETH | ${m['eth_price_usd']:,.0f} | $1,675 | +2.58% (24h) |")
w()
w("### Activity"); w()
...
open("/Users/ls/Documents/MultiversX/projects/onchain-quant-agent/reports/2026-06-15.md","w").write("\n".join(L)+"\n")
print("WROTE reports/2026-06-15.md  lines:",len(L))
