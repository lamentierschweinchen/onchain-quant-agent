#!/usr/bin/env python3
import json
R=json.load(open("/Users/ls/Documents/MultiversX/projects/onchain-quant-agent/reports/2026-06-29.json"))
...
w(f"**Period**: 2026-06-22 -> 2026-06-29 (7 days)")
...
w("|---|---|---|")
w(f"| Price action | DUMP (broad-market) | {d['price_change_pct']:+.2f}% WoW to ${e['egld_price_usd']:.2f}; $2.85 floor broke. BROAD-MARKET beta move (BTC -6.4%, ETH -9.5% WoW), not EGLD-specific decoupling |")
w(f"| Exchange flows | OUTFLOW (streak broke) | Net {wi['exchange_flows']['net_change_egld']:+,.0f} EGLD; Binance hot -158K, Bybit -56K; 3-week inflow streak ended (but distribution shifted to OTC) |")
w(f"| Binance staking custody | STALLED (4th wk) | 3.51M EGLD UNCHANGED; 779K parked (7 weeks, $1.99M); +81K staked rise came from yield-chase, not custody |")
w(f"| Delegator base | STABILIZED (2nd wk) | {si['churn']['delegators_added']:+} WoW; capitulation CONFIRMED a one-shot; base flat 2nd week at 174,407 |")
w(f"| Staking conviction | BUY-THE-DIP | Protocol staked +81K despite the dump; yield-chase REIGNITED (ninjastaking +11.6K, pi-staking +7.1K/+24 users) |")
w(f"| DEX activity | DEPRESSED | {usd(ta['xexchange']['total_volume_24h_usd'])} (+27% WoW on sell-side turnover, but ~half of a month ago) |")
w(f"| OTC pipeline | DISTRIBUTING + LOADING | 195K throughput (record, vs 85K) while desks reloaded +35K; ongoing retail distribution, more loaded |")
...
w(f"| XOXNO LSD | DRAINING | XEGLD supply -29.2% WoW (~94K redeemed); largest single-protocol LSD supply move in tracking; trace next run |")
w(f"| Reward compounding | RECOVERED | Compound rate 58.54% (vs 55.31% last week); reversed the 3-week slide, consistent with buy-the-dip conviction |")
w(f"| Stablecoin supply | CONTRACTING (2nd wk) | USDC -1.3%, USDT -3.7% burn (accelerating); USH -3.2% (CDP de-leveraging resumed); dollar liquidity bridging out |")
w(); w("---"); w()
...
open("/Users/ls/Documents/MultiversX/projects/onchain-quant-agent/reports/2026-06-29.md","w").write("\n".join(L)+"\n")
print("WROTE reports/2026-06-29.md  lines:",len(L))
