#!/usr/bin/env python3
import json
R=json.load(open("/Users/ls/Documents/MultiversX/projects/onchain-quant-agent/reports/2026-07-06.json"))
...
w(f"**Period**: 2026-06-29 -> 2026-07-06 (7 days)")
...
w("|---|---|---|")
w(f"| Price action | BOUNCE (laggard) | {d['price_change_pct']:+.2f}% WoW to ${e['egld_price_usd']:.2f}; first up-week, $2.55 held. Broad relief bounce (BTC +5.2%, ETH +12.3%) but EGLD UNDERPERFORMED (ETH gained 2x+) |")
w(f"| Binance staking custody | UNWOUND (distribution-side) | -158,853 EGLD to hot wallets (traced: +241K in, -400K out); 7-week standoff RESOLVED to drawdown, not delegation |")
w(f"| Exchange flows | OUTFLOW (Binance-only) | Net {wi['exchange_flows']['net_change_egld']:+,.0f} EGLD; ~all Binance (-236K); ex-Binance was net-INFLOW (Crypto.com +27K, Bybit +21K) |")
w(f"| OTC pipeline | DISTRIBUTING + DRAINING | ~172K throughput (vs 195K) while desks DRAINED -8.9K; late-distribution phase, absorbed by buyers |")
w(f"| Distribution absorber | ACCUMULATING | Mega Whale erd18mv2z6r2 +32.7K from Coinbase Routing (2nd week, now 1.04M); a standing bid meeting the distribution |")
w(f"| Delegator base | STABLE (3rd wk) | {si['churn']['delegators_added']:+} WoW; capitulation fully behind us; base flat at 174,373 |")
w(f"| Staking | ROTATION | Delegation TVL +70K broadly (Synexis +18K) but total staked -46K = direct-node -> delegation rotation; yield-chase UNWOUND -9K |")
w(f"| XEGLD supply | STABILIZED | Settled at ~226,400 ({[p for p in da['protocol_breakdown'] if p['protocol']=='XOXNO LSD'][0]['tvl_wow_change_pct']:+.1f}% EGLD); last week's -29% collapse was a ONE-SHOT, no continuation |")
w(f"| DEX activity | DEPRESSED | {usd(ta['xexchange']['total_volume_24h_usd'])} (+11% WoW, tracking the bounce, still ~half of a month ago) |")
...
w(f"| Reward compounding | RECOVERED (2nd wk) | Compound rate 60.35% (vs 58.54% last week); mild bullish drift as price recovered |")
w(f"| Stablecoin supply | DECELERATING | USDC -2.0% (3rd burn wk) but USDT burn nearly stopped (-0.3%, vs -3.7%); USH flat; flight narrowing to USDC |")
w(); w("---"); w()
...
open("/Users/ls/Documents/MultiversX/projects/onchain-quant-agent/reports/2026-07-06.md","w").write("\n".join(L)+"\n")
print("WROTE reports/2026-07-06.md  lines:",len(L))
