#!/usr/bin/env python3
import json
R=json.load(open("/Users/ls/Documents/MultiversX/projects/onchain-quant-agent/reports/2026-07-13.json"))
...
_prevR=json.load(open("/Users/ls/Documents/MultiversX/projects/onchain-quant-agent/reports/2026-07-06.json"))
...
w(f"**Period**: 2026-07-06 -> 2026-07-13 (7 days)")
...
w("|---|---|---|")
w(f"| Price action | BREAKOUT (EGLD-specific) | {d['price_change_pct']:+.2f}% WoW to ${e['egld_price_usd']:.2f}; 2nd up-week, DECOUPLED to the UPSIDE (BTC -0.2%, ETH +0.7% flat). Laggard->leader flip, not yet confirmed |")
w(f"| OTC pipeline | RECORD RELOAD (bearish) | Desks loaded +238K (~98K -> ~335K, biggest in tracking) + record ~323K throughput; distribution wave staged into the strength |")
w(f"| Binance staking | DE-STAKING (2 fronts) | Custody -43K (2nd leg, -202K over 2wks) + binance_staking provider -149K = ~-192K out of stake; hot wallets net +115K inflow |")
w(f"| Exchange flows | OUTFLOW (not clean) | Net {wi['exchange_flows']['net_change_egld']:+,.0f} EGLD; just Bybit -221K (untraced) + UPbit -131K (OTC-routed); rest INFLOW (Binance +115K, Gate +26K) |")
w(f"| Distribution absorber | STEPPED BACK | Mega Whale erd18mv2z6r2 FLAT at 1.04M after 2 accumulation weeks; the identifiable bid went quiet as OTC reloaded |")
w(f"| DeFi leverage | RETURNING (bullish) | USH supply +6.5% (CDPs re-opening), XEGLD +4.5% (LSD recovering), compound rate up 3rd wk to 61.6% |")
w(f"| Delegator base | FLAT (4th wk) | {si['churn']['delegators_added']:+} WoW at 174,349; no new retail on the +16% rip - existing holders re-levering |")
w(f"| Staking | THROUGH THE RALLY | Total staked +80K (holders staking, not chasing spot); delegation -28K NET but ex-binance_staking +120K broad |")
w(f"| XEGLD supply | RE-ACCUMULATING | +4.5% to 236,685; recovering after the -29% collapse - the LSD is back in inflow |")
w(f"| DEX activity | UP ON RIP | {usd(ta['xexchange']['total_volume_24h_usd'])} (+82% WoW, highest since spring); WEGLD/USDC 98% dominant (CEX-derived) |")
...
w(f"| Reward compounding | UP (3rd wk) | Compound rate 61.59% (58.54 -> 60.35 -> 61.59); delegators compounding into strength |")
...
open("/Users/ls/Documents/MultiversX/projects/onchain-quant-agent/reports/2026-07-13.md","w").write("\n".join(L)+"\n")
print("WROTE reports/2026-07-13.md  lines:",len(L))
