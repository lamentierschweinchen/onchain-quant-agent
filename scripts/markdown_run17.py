#!/usr/bin/env python3
import json
R=json.load(open("/Users/ls/Documents/MultiversX/projects/onchain-quant-agent/reports/2026-07-20.json"))
...
_prevR=json.load(open("/Users/ls/Documents/MultiversX/projects/onchain-quant-agent/reports/2026-07-13.json"))
...
w(f"**Period**: 2026-07-13 -> 2026-07-20 (7 days)")
...
w("|---|---|---|")
w(f"| Price action | STALLED / LAGGARD | {d['price_change_pct']:+.2f}% WoW to ${e['egld_price_usd']:.2f} while BTC +1.95% and ETH +4.36% both outpaced it; the run #16 leadership thesis FAILED in one week |")
w(f"| OTC pipeline | DISTRIBUTED (bearish) | Desks DRAINED -255,736 (335K -> 80K, -76%, largest in tracking) on 1,328,037 EGLD true 7d throughput; traced to Bybit/Binance.com via routers |")
w(f"| Exchange flows | INFLOW (reversal) | Net {wi['exchange_flows']['net_change_egld']:+,.0f} EGLD, ending a 3-week outflow streak; UPbit +153K, KuCoin +143K, Bybit +63K - the confirming leg of the distribution |")
w(f"| Single-whale exit | KuCoin +430% | Unknown Whale erd15ku2r2j6 sent its ENTIRE 145,443 EGLD to KuCoin and went to zero; ~0.5% of circulating supply |")
w(f"| Distribution absorber | RESUMED (outgunned) | Mega Whale erd18mv2z6r2 +50,621 to 1.09M via Coinbase Routing; a real bid, but ~1/25th the distributed volume |")
w(f"| Binance staking | DE-STAKING (3rd leg) | Custody -103,579 to 3.21M, -305,549 cumulative from the 3.51M peak; now a programme. binance_staking provider STABILIZED (+298) |")
w(f"| DeFi leverage | PAUSED (neutral) | USH flat +0.14% after last week's +6.5% mint (no new leverage, no forced closures); XEGLD +2.7% still accumulating |")
w(f"| Delegator base | FLAT (5th wk) | {si['churn']['delegators_added']:+} WoW at {si['churn']['total_delegators_current']:,}; five flat weeks across the entire +24% recovery - zero participation broadening |")
w(f"| Staking | GROWING | Total staked +81,022 to 14.62M; delegation +47,701 broad (valuestaking +12.7K, Synexis +11.7K) - existing holders adding |")
w(f"| DEX activity | UP, CONCENTRATED | {usd(ta['xexchange']['total_volume_24h_usd'])} (+20.5% WoW); WEGLD/USDC 91.5% dominant - thin, CEX-derived tape |")
...
w(f"| Reward compounding | SLIPPED | Compound rate 60.19% (was 61.59%); a 3-week rise ended as price stalled |")
w(f"| Stablecoins | STOPPED BLEEDING | USDC -0.13%, USDT +0.05% - multi-week burn ended, but no inflow (no fresh dry powder) |")
w(f"| Stablecoin supply | CHOPPY | USDC -0.66% (4th burn wk, decelerating) but USDT -1.87% (re-accelerated); vs USH minting +6.5% |")
w(); w("---"); w()
...
open("/Users/ls/Documents/MultiversX/projects/onchain-quant-agent/reports/2026-07-20.md","w").write("\n".join(L)+"\n")
print("WROTE reports/2026-07-20.md  lines:",len(L))
