#!/usr/bin/env python3
import json
R=json.load(open("/Users/ls/Documents/MultiversX/projects/onchain-quant-agent/reports/2026-07-27.json"))
...
_prevR=json.load(open("/Users/ls/Documents/MultiversX/projects/onchain-quant-agent/reports/2026-07-20.json"))
...
w(f"**Period**: 2026-07-20 -> 2026-07-27 (7 days)")
...
w("|---|---|---|")
w(f"| Price action | DECOUPLED DOWN | {d['price_change_pct']:+.2f}% WoW to ${e['egld_price_usd']:.2f} while BTC +1.71% and ETH +5.35% both ROSE; chain-specific weakness, recovery reclassified as a bear-market rally |")
w(f"| OTC pipeline | EXHAUSTED (constructive) | Desks drained to 61,495 (through the 80K trigger) on 313,173 EGLD 7d throughput, -76%; critically NO UPbit reload |")
w(f"| Exchange flows | FLAT (once decomposed) | Headline {wi['exchange_flows']['net_change_egld']:+,.0f}, but +156,995 is non-flow (custody reload + untracked-prior wallet); TRUE external flow +9,983 |")
w(f"| Identifiable bid | GONE | Mega Whale erd18mv2z6r2 recorded ZERO value txs (flat at 1,093,312); the Coinbase Routing pipe feeding it drained to 77 EGLD |")
w(f"| Binance staking | RELOADED (reversal) | Custody +150,000 to 3,357,101 - the three-leg de-staking programme (-305,549) broke |")
w(f"| KuCoin whale exit | RESOLVED BEARISH | The 145,443 deposit STAYED (-2.6% only); per run #17's pre-commitment it was sold into the book, not withdrawn |")
w(f"| DeFi leverage | UNWINDING | USH burned -2.60% past the 1% threshold; run #11 de-leveraging rule re-activated, run #16 mint confirmed a chase |")
w(f"| DeFi deposits | STRONGEST EVER | Hatom Lending EGLD-TVL +9.89% vs price -10.13%; inverse ratio 0.98, best on record - run #11's capacity-decay hypothesis falsified |")
w(f"| Delegator base | INERT (6th wk) | {si['churn']['delegators_added']:+} WoW at {si['churn']['total_delegators_current']:,}; PROMOTED to a structural finding at the pre-registered 6-week threshold |")
w(f"| Staking | UNSTAKING | Total staked {d['staked_egld_added']:+,} to {e['staked_egld']:,}; ~110K of it left DIRECT-NODE stake, delegation only -16,634 |")
w(f"| DEX activity | COLLAPSED | {usd(ta['xexchange']['total_volume_24h_usd'])} (-61.1% WoW, 2nd-largest drop in tracking) on {ta['xexchange']['top_pair_dominance_pct']:.1f}% WEGLD/USDC; pool TVL ROSE - depth stayed, trading left |")
...
w(f"| Reward compounding | SLIPPED (2nd wk) | Compound rate 58.81% (was 60.19%) |")
w(f"| Stablecoins | BLEED RESUMED | USDC -1.01%, USDT -2.79% after exactly one flat week - the pause was a function of the rally |")
w(); w("---"); w()
...
if "reward_behavior" in si:
    ...
    w("### Reward Behavior"); w()
    w(f"- **Compound rate** (function-level): {rb.get('compound_pct_at_function_level','n/a')}% (vs 60.19% last run; 2nd consecutive weekly slip, from a 61.59% peak in run #16)")
    w()
...
open("/Users/ls/Documents/MultiversX/projects/onchain-quant-agent/reports/2026-07-27.md","w").write("\n".join(L)+"\n")
print("WROTE reports/2026-07-27.md  lines:",len(L))
