#!/usr/bin/env python3
import json
R=json.load(open("/Users/ls/Documents/MultiversX/projects/onchain-quant-agent/reports/2026-08-03.json"))
...
_prevR=json.load(open("/Users/ls/Documents/MultiversX/projects/onchain-quant-agent/reports/2026-07-27.json"))
...
w(f"**Period**: 2026-07-27 -> 2026-08-03 (7 days)")
...
w(f"| Price action | DOWN WITH THE TAPE | {d['price_change_pct']:+.2f}% WoW to ${e['egld_price_usd']:.2f} with BTC {100*(m['btc_price_usd']-pecon['btc_price_usd'])/pecon['btc_price_usd']:+.2f}% and ETH {100*(m['eth_price_usd']-pecon['eth_price_usd'])/pecon['eth_price_usd']:+.2f}%; no repeat of run #18's decoupling - broad risk-off |")
w(f"| OTC pipeline | CIRCULAR (80% churn) | Gross {otc['gross_outbound_egld_7d']:,.0f} EGLD but only {otc['net_one_way_egld_7d']:,.0f} genuine one-way; Bybit and Gate.io feed the desks they receive from |")
w(f"| OTC feed | UPbit RESUMED | Fresh {otc['upbit_reload_egld']:,.0f} EGLD tranche after zero last week; desk balance {otc['desk_balance_egld']-otc['previous_desk_balance_egld']:+,.0f} to {otc['desk_balance_egld']:,.0f} |")
w(f"| Exchange flows | CLEAN OUTFLOW | {wi['exchange_flows']['net_change_egld']:+,.0f} EGLD and for once no adjustment needed - no custody reload, no newly tracked address |")
w(f"| Identifiable bid | ABSENT (2nd wk) | Mega Whale erd18mv2z6r2 zero value txs, unchanged at {dem['mega_whale_balance_egld']:,.0f}; Coinbase Routing pipe still at {dem['coinbase_routing_balance_egld']:,.0f} EGLD |")
w(f"| Binance staking | DE-STAKING INTACT | Last week's +150,000 traced to an unDelegate/withdraw on the delegation contract - run #18's 'reversal' is FALSIFIED; custody flat |")
w(f"| DEX depth | TURNOVER HALVED AGAIN | {x0['previous_turnover_ratio_pct']:.2f}% -> {x0['turnover_ratio_pct']:.2f}% of pool TVL traded daily; depth flat, traders gone |")
w(f"| DEX activity | NEW TRACKED LOW | {usd(x0['total_volume_24h_usd'])} 24h volume on {x0['top_pair_dominance_pct']:.1f}% WEGLD/USDC concentration |")
w(f"| DeFi leverage | UNWINDING, ORDERLY | USH burned -2.93% for a 2nd week (cumulative -5.5%); no forced-closure acceleration, so no capitulation tell |")
w(f"| DeFi deposits | ADDING (2nd wk) | Hatom Lending EGLD-TVL +3.04% vs price -5.28%; inverse ratio 0.58, 7th confirmation, mid-range |")
w(f"| Liquid staking | FLAT (both legs) | SEGLD -0.23%, XEGLD -0.15% - XOXNO held through a drawdown for the first time in four |")
w(f"| Staking | ROTATION, WEEK 2 | Total staked {d['staked_egld_added']:+,} to {e['staked_egld']:,} while delegation GREW +56,300; ~79.5K left DIRECT-NODE stake |")
w(f"| Yield arbitrage | THE ONLY ACTIVE FORCE | pi-staking +6,449 (7th wk) at 9.10% APR / 0% fee; APR>=8.8% cohort +37,090 while 20%-fee incumbents shed |")
w(f"| Delegator base | INERT (7th wk) | {si['churn']['delegators_added']:+} WoW at {si['churn']['total_delegators_current']:,}; background assumption since run #18 |")
...
w(f"| Stablecoins | OUTFLOW STOPPED | USDC -0.17%, USDT -0.02% - smallest combined move in the series, but still no inflow |")
w(); w("---"); w()
...
w(f"Gross desk throughput was **{otc['gross_outbound_egld_7d']:,.0f} EGLD** out / {otc['gross_inbound_egld_7d']:,.0f} in. Resolving BOTH legs two hops out shows **{otc['circular_share_pct']:.0f}% is round-trip churn** between the desks and the venues that feed them, leaving **{otc['net_one_way_egld_7d']:,.0f} EGLD of genuine one-way movement**.")
w()
...
w()
w(f"_Gross series (runs #12-#19, paginated, NOT netted for circularity): " +
  " / ".join(f"{k[3:]}: {v:,.0f}" for k,v in otc["gross_series_egld_7d"].items()) + "._")
w()
w(f"_{otc['series_note']}_")
w()
w("### Demand Instruments (NEW this run)"); w()
w("| Instrument | Reading | Prior |")
w("|---|---|---|")
w(f"| Identifiable bid absorbed (7d) | {dem['identifiable_bid_absorbed_egld_7d']:,.0f} EGLD | 0 (week {dem['weeks_at_zero']} at zero) |")
w(f"| Mega Whale erd18mv2z6r2 | {dem['mega_whale_balance_egld']:,.0f} EGLD ({dem['mega_whale_change_egld']:+,.0f}) | zero value txs |")
w(f"| Coinbase Routing wallet | {dem['coinbase_routing_balance_egld']:,.0f} EGLD | dry 2nd week |")
...
w(f"| Withdrawal breadth (>1K EGLD, ex-pipeline) | {b['distinct_recipients_ex_pipeline']} addresses / {b['total_egld_ex_pipeline']:,.0f} EGLD | first measurement |")
w(f"| Withdrawal breadth (raw) | {b['distinct_recipients_raw']} addresses / {b['total_egld_raw']:,.0f} EGLD | {b['pipeline_share_pct']:.0f}% went to the OTC pipeline |")
w()
...
open("/Users/ls/Documents/MultiversX/projects/onchain-quant-agent/reports/2026-08-03.md","w").write("\n".join(L)+"\n")
print("WROTE reports/2026-08-03.md  lines:",len(L))
