...
_hotpipe=[a for a in an if a['metric']=='binance_hot_to_otc_feeders_egld'][0]
w(f"| Binance feed | STANDING PROGRAMME, 5x THRESHOLD | Hot wallet -> known OTC feeders {_hotpipe['current_value']:,.0f} EGLD over the full week ({100*_hotpipe['current_value']/otc['net_one_way_egld_7d']:.0f}% of everything delivered). But custody ROSE +{372434:,} after two weeks of drawdown - the feed is not funded by the custody balance |")
