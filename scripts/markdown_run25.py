#!/usr/bin/env python3
import json
R=json.load(open("/Users/ls/Documents/MultiversX/projects/onchain-quant-agent/reports/2026-09-14.json"))
...
_prevR=json.load(open("/Users/ls/Documents/MultiversX/projects/onchain-quant-agent/reports/2026-09-07.json"))
...
w(f"**Period**: 2026-09-07 -> 2026-09-14 (7 days)")
...
w(f"| MEX / xExchange | MEX/WEGLD POOL PAUSED | Paused 2026-09-13 16:43 UTC by the router owner; {mexe['pair_holds_mex']/1e9:,.1f}B MEX + {mexe['pair_holds_wegld']:,.0f} WEGLD locked, 172 failed txs. MEX {x0['mex_price_change_wow_pct']:+.0f}% WoW (CoinGecko); HMEX supply {100*(mexe['hmex_supply']-mexe['hmex_prev_supply'])/mexe['hmex_prev_supply']:+.0f}% |")
w(f"| Price action | EGLD-SPECIFIC REVERSAL | {d['price_change_pct']:+.2f}% WoW to ${e['egld_price_usd']:.2f}; BTC {100*(m['btc_price_usd']-pecon['btc_price_usd'])/pecon['btc_price_usd']:+.2f}%, ETH {100*(m['eth_price_usd']-pecon['eth_price_usd'])/pecon['eth_price_usd']:+.2f}% |")
w(f"| OTC feed | WAVE #4 CONFIRMED | UPbit tranche {otc['upbit_reload_egld']:,.0f}; net one-way {otc['net_one_way_egld_7d']:,.0f}; desks {otc['previous_desk_balance_egld']:,.0f} -> {otc['desk_balance_egld']:,.0f} |")
w(f"| Pipeline scale | ~2% OF CEX SPOT VOLUME | Delivery = {ob['net_one_way_share_of_spot_volume_pct']:.1f}% of {ob['spot_volume_7d_egld']/1e6:,.2f}M EGLD 7d spot volume (last week {ob['previous_net_one_way_share_of_spot_volume_pct']:.1f}%) |")
w(f"| Order book (NEW) | FIRST READING | Binance ±2% ${ob['binance']['depth_plus2_usd']:,.0f} ask / ${ob['binance']['depth_minus2_usd']:,.0f} bid; Bybit ${ob['bybit']['depth_plus2_usd']:,.0f} / ${ob['bybit']['depth_minus2_usd']:,.0f} |")
w(f"| Netting frame | 4-WEEK WINDOW | Aug 17 - Sep 14 net {wave['net_one_way_egld']:,.0f} vs weekly sum {wave['sum_of_weekly_nets_egld']:,.0f} ({wave['weekly_frame_overstatement_pct']:.0f}%); circularity {otc['circular_share_pct']:.0f}% below band |")
w(f"| Staked ratio | BROKE 46.80% | {100*e['staked_ratio']:.2f}% ({d['staked_ratio_change_pp']:+.2f}pp); staked {d['staked_egld_added']:+,}; new unDelegations {ub['queue_this_week']['undelegated_egld']:,.0f} (from 208,125) |")
w(f"| Compound rate | BELOW 50% FIRST TIME | {rbx['compound_pct_at_function_level']:.2f}% ({rbx['compound_vs_claim']['redelegate_count']} vs {rbx['compound_vs_claim']['claim_count']}); claims held, not sold |")
w(f"| Binance feed | PERMANENT LINE | Hot wallet -> OTC feeders, day-sliced, third week above 50K; custody refilled again |")
w(f"| Feeder attribution | RESOLVED | The 196K unattributed feeder traces to Bybit and Gate.io, not a fifth venue |")
w(f"| Stablecoins | PROMOTION WITHDRAWN | Combined expansion; USDT -12.8%, USDC +2.7% |")
w(f"| Delegator base | FLAT, 13TH WEEK | {si['churn']['delegators_added']:+,} to {si['churn']['total_delegators_current']:,}; {si['churn']['providers_gaining_delegators']} gaining / {si['churn']['providers_losing_delegators']} losing |")
w(f"| Identity renames | JOIN TRAP CAUGHT | cslabsio -> chainstatelabs (177,587 EGLD) and kevinlallement -> dinovox would have posed as exits |")
w(f"| DEX | ONE-PAIR VENUE | {x0['dex_volume_egld_24h']:,.0f} EGLD/day ({x0['dex_vol_egld_wow_pct']:+.0f}%), {x0['wegld_usdc_share_of_volume_pct']:.1f}% WEGLD/USDC |")
w(f"| Withdrawal breadth | DISPERSAL CONFIRMED | {dem['withdrawal_breadth']['distinct_recipients_ex_pipeline']} recipients / {dem['withdrawal_breadth']['total_egld_ex_pipeline']:,.0f} EGLD, top two {dem['withdrawal_breadth']['top_two_share_pct']:.0f}% |")
w(f"| New tokens | 2 CLEAR THE BAR | First qualifying launches in nine weeks |")
w(); w("---"); w()
...
w()
w("### OTC Pipeline - wave #4 on a near-empty warehouse"); w()
w(f"Gross {otc['gross_outbound_egld_7d']:,.0f} out / {otc['gross_inbound_egld_7d']:,.0f} in, {otc['circular_share_pct']:.0f}% circular, **{otc['net_one_way_egld_7d']:,.0f} EGLD one-way**. UPbit fed **{otc['upbit_reload_egld']:,.0f}**; inventory {otc['previous_desk_balance_egld']:,.0f} -> **{otc['desk_balance_egld']:,.0f}**. Netted over Aug 17 - Sep 14 as one window: **{wave['net_one_way_egld']:,.0f} EGLD**.")
w()
...
w()
w(f"**Aug 17 - Sep 14 window, feed-to-drain:**"); w()
...
w("|---|---|")
w(f"| CEX 7d spot volume (NEW) | {ob['spot_volume_7d_egld']:,.0f} EGLD (prior {ob['spot_volume_prior_7d_egld']:,.0f}) |")
w(f"| Delivery share of spot (NEW) | {ob['net_one_way_share_of_spot_volume_pct']:.2f}% (last week {ob['previous_net_one_way_share_of_spot_volume_pct']:.2f}%) |")
...
open("/Users/ls/Documents/MultiversX/projects/onchain-quant-agent/reports/2026-09-14.md","w").write("\n".join(L)+"\n")
print("WROTE reports/2026-09-14.md  lines:",len(L))
