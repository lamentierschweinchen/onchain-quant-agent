#!/usr/bin/env python3
import json
R=json.load(open("/Users/ls/Documents/MultiversX/projects/onchain-quant-agent/reports/2026-05-25.json"))
...
e=nh["economics"]; d=nh["deltas"]; pecon=json.load(open("/Users/ls/Documents/MultiversX/projects/onchain-quant-agent/data/previous.json"))["economics"]
...
w(f"**Period**: 2026-05-18 → 2026-05-25 (7 days)")
...
w("|---|---|---|")
w(f"| Price regime | 🟢 Recovering | {d['price_change_pct']:+.1f}% WoW to ${e['egld_price_usd']:.2f}; z=-0.27σ (normal) |")
w(f"| Exchange flows | 🟢 Reversed | Net {wi['exchange_flows']['net_change_egld']:+,.0f} EGLD (outflow); +169K inflow last week did not continue |")
w(f"| Binance staking custody | 🟡 Accumulating | 3.38M EGLD (+267K parked, not delegated) |")
w(f"| DEX activity | 🟡 Composition shift | ${ta['xexchange']['total_volume_24h_usd']/1e3:.0f}K; ZPAY pair 40.8%, WEGLD/USDC down to 56.2% |")
w(f"| Staking concentration | 🟢 Healthy | HHI {si['concentration']['hhi']:.3f}; top-5 {si['concentration']['top_5_share_pct']:.1f}% |")
w(f"| Yield-chase migration | 🟡 Regime shift wk4 | procryptostaking +17K, valuestaking +16K |")
w(f"| Delegator base | 🟡 Flat 5 wks | {si['churn']['total_delegators_current']:,} (-53); whale consolidation |")
w(f"| Staked ratio | 🟢 Stabilized | {e['staked_ratio']*100:.2f}% (z=-1.38σ); no anomaly breach |")
w(f"| Token holder growth | 🔴 -9 weeks | Continued small declines across all top-10 |")
w(); w("---"); w()
...
w(f"| BTC | ${m['btc_price_usd']:,} | $77,038 | +0.26% |")
w(f"| ETH | ${m['eth_price_usd']:,.0f} | $2,120 | -0.29% |")
w()
w("### Activity"); w()
...
w("|---|---|---|---|")
for t in wi["large_transactions"][:12]:
...
for p in si["top_providers"][:12]:
    ...
    w(f"| {p['rank']} | {p['identity']} | {egld(p['locked_egld'])} | {p['apr_pct']:.2f}% | {p['fee_pct']*100:.1f}% | {wow} |")
w()
...
w(f"**Validator movements:** {vm['providers_joining']} joining, {vm['providers_leaving']} leaving, net {vm['net_provider_change']}. Quiet week — no confident large named movements after run #8's MrsEGLD 158K joiner.")
w(); w("---"); w()
w("## Watch List"); w()
...
open("/Users/ls/Documents/MultiversX/projects/onchain-quant-agent/reports/2026-05-25.md","w").write("\n".join(L)+"\n")
print("WROTE reports/2026-05-25.md  lines:",len(L))
